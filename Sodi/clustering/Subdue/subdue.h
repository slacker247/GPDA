...
// Assume that UNIX flavor is used if WIN32 is not defined
#ifndef WIN32
#undef UNIX
#define UNIX
#endif
...
#define CLA_NO_MIN_ENCODE		"-nominencode"       /* deprecated */
...
#define CL_HIERARCHY_FILE					"clHierarchy"
...
typedef union _LABEL_CONTENT
{
	char *stringValue;
	DOUBLE numericValue;
} LABEL_CONTENT, *PLABEL_CONTENT;

typedef union _LABEL_MATCH
{
...
typedef struct _SUB_GRAPH_EDGE
{
	ULONG indexInGraphVertex;
} SUB_GRAPH_EDGE, *PSUB_GRAPH_EDGE;

typedef struct _SUB_GRAPH_VERTEX
{
	ULONG indexInGraph;
	ULONG numberOfEdges;
	ULONG edgesIndex;
} SUB_GRAPH_VERTEX, *PSUB_GRAPH_VERTEX;

typedef struct _SUB_GRAPH
{
...
//	ULONG numberOfUndirectedEdges;
	...
	ULONG refCount;
	ULONG numberInSub;
} SUB_GRAPH, *PSUB_GRAPH;

typedef struct _LIST_OF_SUB_GRAPHS_NODE	
{
	PSUB_GRAPH subGraph;
	struct  _LIST_OF_SUB_GRAPHS_NODE *next;
} LIST_OF_SUB_GRAPHS_NODE, *PLIST_OF_SUB_GRAPHS_NODE;

typedef struct _LIST_OF_SUB_GRAPHS
{
	ULONG currentLength;
	PLIST_OF_SUB_GRAPHS_NODE currentNode;
	PLIST_OF_SUB_GRAPHS_NODE head;
} LIST_OF_SUB_GRAPHS, *PLIST_OF_SUB_GRAPHS;
...
typedef struct _LIST_OF_SUBS_NODE
{
	PSUB sub;
	struct _LIST_OF_SUBS_NODE *next;
	struct _LIST_OF_SUBS_NODE *previous;
} LIST_OF_SUBS_NODE, *PLIST_OF_SUBS_NODE;

typedef struct _LIST_OF_SUBS
{
	ULONG maxLength;
	ULONG currentLength;
...
} LIST_OF_SUBS, *PLIST_OF_SUBS;

typedef struct _GRAPH_TEMPLATE
{
	ULONG *verticesTemplate;
	ULONG **edgesTemplate;
} GRAPH_TEMPLATE, *PGRAPH_TEMPLATE;

typedef struct _MATCHED_PAIR
{
	ULONG first;
	ULONG second;
} MATCHED_PAIR, *PMATCHED_PAIR;

typedef MATCHED_PAIR UNMATCHED_PAIR;

typedef PMATCHED_PAIR PUNMATCHED_PAIR;

typedef struct _MATCH_QUEUE_NODE
{
	LONG depth;
	DOUBLE matchCost;
...
} MATCH_QUEUE_NODE, *PMATCH_QUEUE_NODE;

typedef struct _GLOBAL_MATCH_QUEUE
{
	PMATCH_QUEUE_NODE QHead;
	PMATCH_QUEUE_NODE QTail;
} GLOBAL_MATCH_QUEUE, *PGLOBAL_MATCH_QUEUE;

typedef struct _LOCAL_MATCH_QUEUE
{
	PMATCH_QUEUE_NODE QHead;
	PMATCH_QUEUE_NODE QTail;
} LOCAL_MATCH_QUEUE, *PLOCAL_MATCH_QUEUE;

typedef struct _REVERSE_SUB_GRAPH_NODE
{
	ULONG vertexIndex;
	BOOLEAN used;
	PGRAPH_EDGE graphEdge;
	struct _REVERSE_SUB_GRAPH_NODE *nextNode;
} REVERSE_SUB_GRAPH_NODE, *PREVERSE_SUB_GRAPH_NODE;

typedef struct _REVERSE_SUB_GRAPH
{
	ULONG *totalFan;
	PREVERSE_SUB_GRAPH_NODE *reverseVerticesEdges;
	PREVERSE_SUB_GRAPH_NODE *forwardVerticesEdges;
} REVERSE_SUB_GRAPH, *PREVERSE_SUB_GRAPH;

typedef struct _EXPANDED_INSTANCE_VE
{
...
} EXPANDED_INSTANCE_VE, *PEXPANDED_INSTANCE_VE;

struct _VE_TEMPLATE;

typedef struct _CONSISTENT_VE_LIST_NODE
{
	struct _VE_TEMPLATE *Template;
	LONG *matchingPattern;
	struct _CONSISTENT_VE_LIST_NODE *next;
} CONSISTENT_VE_LIST_NODE, *PCONSISTENT_VE_LIST_NODE;

typedef struct _VE_TEMPLATE
{
...
} VE_TEMPLATE, *PVE_TEMPLATE;

typedef struct _EXPANDED_INSTANCE_E
{
	PSUB_GRAPH parentInstance;
	ULONG sourceVertexIndex;
	ULONG targetVertexIndex;
...
} EXPANDED_INSTANCE_E, *PEXPANDED_INSTANCE_E;

struct _E_TEMPLATE;

typedef struct _CONSISTENT_E_LIST_NODE
{
	struct _E_TEMPLATE *Template;
	LONG *matchingPattern;
	struct _CONSISTENT_E_LIST_NODE *next;
} CONSISTENT_E_LIST_NODE, *PCONSISTENT_E_LIST_NODE;

typedef struct _E_TEMPLATE
{
	ULONG numberOfInstances;
	ULONG newEdgeLabelIndex;
	ULONG sourceVertexMatchIndex;
...
} E_TEMPLATE, *PE_TEMPLATE; 

typedef struct _EDGE_LABEL_TEMPLATES
{
	PE_TEMPLATE ETemplates;
	PVE_TEMPLATE VETemplates;
} EDGE_LABEL_TEMPLATES, *PEDGE_LABEL_TEMPLATES;

typedef struct _ABSTRACT_SUB_VERTEX
{
	ULONG labelIndex;
	ULONG edgesIndex;
	ULONG numberOfEdges;
	ULONG covered;
} ABSTRACT_SUB_VERTEX, *PABSTRACT_SUB_VERTEX;

typedef struct _ABSTRACT_SUB_EDGE
{
	ULONG labelIndex;
	ULONG targetVertex;
	BOOLEAN directed;
	ULONG covered;
} ABSTRACT_SUB_EDGE, *PABSTRACT_SUB_EDGE;

typedef struct _ABSTRACT_SUB
{
	ULONG numberOfVertices;
	ULONG numberOfEdges;
//{U}
...
} ABSTRACT_SUB, *PABSTRACT_SUB;

typedef struct _ABSTRACT_SUB_NODE
{
	PABSTRACT_SUB abstractSub;
	struct _ABSTRACT_SUB_NODE *next;
} ABSTRACT_SUB_NODE, *PABSTRACT_SUB_NODE;

typedef struct _LIST_OF_ABSTRACT_SUBS
{
	PABSTRACT_SUB_NODE head;
	PABSTRACT_SUB_NODE tail;
} LIST_OF_ABSTRACT_SUBS, *PLIST_OF_ABSTRACT_SUBS;

typedef struct _NEGATIVE_SUB
{
	ULONG negativeSubID;
	PABSTRACT_SUB abstractSub;
	PSUB sub;
	struct _NEGATIVE_SUB *next;
} NEGATIVE_SUB, *PNEGATIVE_SUB;
...
typedef struct _GRAPH_VARIABLES
{
	...
	ULONG	subNumber;							// current iteration
	ULONG	maxNumberOfBestSubs;				// number of subs to print
	...
	BOOLEAN	edge_e_Directed;					// if e-type edges are directed or not
	...
	BOOLEAN prune2;
	ULONG	prune2Value;
	ULONG   evalType;			// OLD: Dr Holder's, New: Istvan's (default)
	BOOLEAN saveBestSubDef;		// TRUE: save the definition of the best sub in file Sub_x
	...
	FILE	*clHierarchyFile;			// hierarchy of the classification lattice
...
} GRAPH_VARIABLES, *PGRAPH_VARIABLES;
...
void CL_TraverseInit(PCLASSIFICATION_LATTICE clNode);
...
//void CountUsedLabels(PGRAPH_VARIABLES GV);
...
void FindMinMax( PGRAPH_VARIABLES GV, PLIST_OF_SUB_GRAPHS subGraphsList,
				PRINT_MODE mode, ULONG vertexIndex, ULONG edgeIndex, 
                ULONG originalTarget, DOUBLE min, DOUBLE max );
void RangePrintSubGraph( PGRAPH_VARIABLES GV, PSUB_GRAPH subGraph, PRINT_MODE mode,  BOOLEAN negativeSub, PSUB sub );
...
void SaveSub( PGRAPH_VARIABLES GV, PSUB sub);
void SaveInstance( PGRAPH_VARIABLES GV, PSUB_GRAPH sub, CHAR *fileName );
