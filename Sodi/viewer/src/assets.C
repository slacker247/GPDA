...
int           coverage;                 // Global coverage flag. Overrides each sensor flag
int           gbifixed;                 // True if stationary sensor
...
C_BASETYPE      *gbi_sites, *site, *area, *sensor_type, *basetype, *parameters;
...
*/ 
   ...
   parameters = gbi_parser->get_basetype("parameters");
   coverage   = (int)parameters->get_logical("coverage");
   ...
   for (k=0; k<gbicount; k++) {
	...
	gbifixed            = radar_name->get_logical("fixed");
        ...
        X[1] = Assets[i].gbiy;
        X[2] = Assets[i].gbiz;
	Assets[i].sensorobj->set_llah(Assets[i].gbilatit*M_PI/180.0,
		                      Assets[i].gbilongi*M_PI/180.0,
				      Assets[i].gbialt, 0.0);
	Assets[i].sensorobj->set_xyz(Assets[i].gbix, Assets[i].gbiy, Assets[i].gbiz);
	...
	values[5]    = (float)gbifixed;
	int sencover = (gbicover && coverage);
        Assets[i].gbisensor = new GR_Sensor(1900+i, Assets[i].gbistn, Assets[i].gbistype, 
                                      (float)latitude, (float)longitude, values, sencover);
	//Assets[i].gbisensor->set_cover((gbicover && coverage));
        sensor_displist->add_object(Assets[i].gbisensor);
        if (INFOfp != NULL)
           fprintf(INFOfp, "ASSETS:  Putting a sensor at %f %f\n", latitude, longitude);
     } else Assets[i].gbistype = -1;
     //
     //   Put the label on the display
     //
     if ((gbilabel != 0) && gbifixed) {
