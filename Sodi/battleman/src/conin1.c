/* conin1.f -- translated by f2c (version 19960717).
...
/* Subroutine */ int conin1_()
{
    ...
    /* Local variables */
    static integer i__;
    static real xmean[24], ci_temp__[24], cl_temp__[24], cw_temp__[24], 
	    si_temp__[24], sl_temp__[24], sw_temp__[24];
...
/*   convert longitude of ascending node to radians */
/*   convert argument of perigee to radians */

    i__1 = shared_1.num_pebbles__;
    for (i__ = 1; i__ <= i__1; ++i__) {
	if (shared_1.pebble[i__ - 1]) {
	    odata_1.orbalt[i__ - 1] = odata_1.orbalt[i__ - 1] * (float)1e3 + 
		    shared_1.rearth;
	    odata_1.orblan[i__ - 1] *= shared_1.degtorad;
	    odata_1.orbaop[i__ - 1] *= shared_1.degtorad;

/*   calculate period and orbital velocity (rad/sec) */

	    d__1 = (doublereal) odata_1.orbalt[i__ - 1];
	    odata_1.angv[i__ - 1] = shared_1.sqrt_mu__ / pow_dd(&d__1, &c_b3);

/*   evaluate components of rotation matrix for each orbital plane
 */
/*   l= cap omega, w= small omega, i= inclination angle */

	    cl_temp__[i__ - 1] = cos(odata_1.orblan[i__ - 1]);
	    sl_temp__[i__ - 1] = sin(odata_1.orblan[i__ - 1]);
	    cw_temp__[i__ - 1] = cos(odata_1.orbaop[i__ - 1]);
	    sw_temp__[i__ - 1] = sin(odata_1.orbaop[i__ - 1]);
	    ci_temp__[i__ - 1] = cos(odata_1.orbinc[i__ - 1]);
	    si_temp__[i__ - 1] = sin(odata_1.orbinc[i__ - 1]);

	    odata_1.rot_1__[i__ - 1] = cl_temp__[i__ - 1] * cw_temp__[i__ - 1]
		     - sl_temp__[i__ - 1] * sw_temp__[i__ - 1] * ci_temp__[
		    i__ - 1];
	    odata_1.rot_4__[i__ - 1] = -cl_temp__[i__ - 1] * sw_temp__[i__ - 
		    1] - sl_temp__[i__ - 1] * cw_temp__[i__ - 1] * ci_temp__[
		    i__ - 1];
	    odata_1.rot_2__[i__ - 1] = sl_temp__[i__ - 1] * cw_temp__[i__ - 1]
		     + cl_temp__[i__ - 1] * sw_temp__[i__ - 1] * ci_temp__[
		    i__ - 1];
	    odata_1.rot_5__[i__ - 1] = -sl_temp__[i__ - 1] * sw_temp__[i__ - 
		    1] + cl_temp__[i__ - 1] * cw_temp__[i__ - 1] * ci_temp__[
		    i__ - 1];
	    odata_1.rot_3__[i__ - 1] = sw_temp__[i__ - 1] * si_temp__[i__ - 1]
		    ;
	    odata_1.rot_6__[i__ - 1] = cw_temp__[i__ - 1] * si_temp__[i__ - 1]
		    ;

	    xmean[i__ - 1] = sdata_1.sataop[i__ - 1] + shared_1.dphase * 
		    odata_1.angv[i__ - 1];
	    if (xmean[i__ - 1] > shared_1.twopi) {
		sdata_1.xmaly[i__ - 1] = xmean[i__ - 1] - shared_1.twopi;
	    } else {
		sdata_1.xmaly[i__ - 1] = xmean[i__ - 1];
	    }
	}
/* L100: */
    }
