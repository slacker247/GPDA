...
#+ALLEGRO (setf excl:*redefinition-warnings* (delete :OPERATOR excl:*redefinition-warnings*))

;;;(defconstant *dlr-version-number* "1.4")
...
(defun dlr-toprel (n)
  (if (and (integerp n) (> n 0))
      (intern (format nil "TOP~A" (dlr-set-max-arity n)))))

(defun dlr-chnum (ch)
  (- (char-code ch) (char-code #\0)))
		     
(defun dlr-rn (n)
  (intern (format nil "U~A" n)))
...
(defun dlr-translate-rel (r)
  (if (atom r)
	...
	(if (and (= (length rn) 4) (string-equal rn "Top" :end1 3) 
		 (>= (dlr-chnum (elt rn 3)) 1) (<= (dlr-chnum (elt rn 3)) 9))
	    (let ((rel (dlr-toprel (dlr-chnum (elt rn 3)))))
	      (if (not (g-concept rel)) (grail-define-primconcept rel))
		rel))
	(if (not (g-concept r))
	    (error "~&!!ERROR!! RELATION NOT YET DEFINED - ~S~%" r)
	  r))
    (if (endp r)
	(error "~&!!ERROR!! BAD RELATION SYNTAX - ~S~%" r)
      (case (car r)
	((and or) 
	 (cons (first r) (mapcar #'dlr-translate-rel (rest r))))
	(not 
	 (list 'not (dlr-translate-rel (second r))))
	(T
	 (if (and (= (length r) 3) (dlr-rel-role (first r) (second r)) 
		  (dlr-toprel (second r)))
...
(defun dlr-translate-con (c)
  (if (atom c)
	...
	(error "bad concept syntax ~S" c)
      (case (car c)
	((and or) (cons (first c) (mapcar #'dlr-translate-con (rest c))))
	(not (list 'not (dlr-translate-con (second c))))
	((some all) (list (first c)
	...
	(atmost
	 (if (and (= (length c) 4) (integerp (second c))
		 ...
		 (let ((pc (intern (format nil "ATMOST-~A-INV-~A-~A"
...
;;; FaCT stuff
  ...
  (setf (g-concept :TOP) (make-grail-concept :name '*TOP* :definition '*TOP*))
  (setf (g-concept :BOTTOM) (make-grail-concept :name '*BOTTOM* :definition '*BOTTOM*))
