...
ULONG StoreLabel (Label *label, LabelList *labelList)
{
  ...
  if (labelIndex == labelList->numLabels) { // i.e., label not found
	...
	(sizeof (char) * (strlen (label->labelValue.stringLabel)) + 1);
      if (stringLabel == NULL)
...
ULONG SubLabelNumber (ULONG index, LabelList *labelList)
{
  ...
  match = TRUE;
  subNumber = 0;
  if (labelList->labels[index].labelType == STRING_LABEL) { // string label?
...
//
// INPUTS: (FILE *outFile, ULONG index, LabelList *labelList)
//
// RETURN: (void)
//
// PURPOSE: Write label labelList[index] to given file.
//---------------------------------------------------------------------------
...
void WriteLabelToFile (FILE *outFile, ULONG index, LabelList *labelList)
{
  ...
  switch (labelType) {
  case STRING_LABEL:
    fprintf (outFile, "%s", labelList->labels[index].labelValue.stringLabel);
    break;
  case NUMERIC_LABEL:
