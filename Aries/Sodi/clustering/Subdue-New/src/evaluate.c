...
void EvaluateSub (Substructure *sub, Parameters *parameters)
{
  ...
  case EVAL_MDL:
    ...
    if (negGraph != NULL) {
      ...
      compressedGraph = CompressGraph (negGraph, sub->negInstances,
				       parameters);
      sizeOfCompressedNegGraph = MDL (compressedGraph, numLabels, parameters);
      // add extra bits to describe where external edges connect to instances
  ...
  case EVAL_SIZE:
    ...
    sizeOfCompressedPosGraph =
      (double) SizeOfCompressedGraph (posGraph, sub->instances, parameters);
    subValue = sizeOfPosGraph / (sizeOfSub + sizeOfCompressedPosGraph);
    if (negGraph != NULL) {
...
//
// PURPOSE: Compute the number of different vertices that vertex v has
// an edge to.  If edge is undirected, then it is included only if
// connected to a larger-numbered vertex (or itself).  This prevents
...
ULONG MaxEdgesToSingleVertex (Graph *graph, ULONG v1)
{
  ...
  for (i = 0; i < numEdges; i++) {
    ...
    if (edge1->vertex1 == v1)
      v2 = edge1->vertex2;
    else v2 = edge1->vertex1;
    if (((edge1->directed) && (edge1->vertex1 == v1)) || // outgoing edge
	((! edge1->directed) && (v2 >= v1))) {
      // count how many edges to v2
      numEdgesToVertex2 = 1;
      for (j = i + 1; j < numEdges; j++) { // start at i+1 since only want max
	edge2 = & graph->edges[graph->vertices[v1].edges[j]];
	if (edge2->vertex1 == v1)
	  v2j = edge2->vertex2;
	else v2j = edge2->vertex1;
	if (v2j == v2) {
	  if (((edge2->directed) && (edge2->vertex1 == v1)) || // outgoing edge
...
ULONG PosExamplesCovered (Substructure *sub, Parameters *parameters)
{
  return ExamplesCovered (sub->instances,
			  ...
			  parameters->posEgsVertexIndices);
}
...
ULONG NegExamplesCovered (Substructure *sub, Parameters *parameters)
{
  return ExamplesCovered (sub->negInstances,
			  ...
			  parameters->negEgsVertexIndices);
}
...
ULONG ExamplesCovered (InstanceList *instanceList, Graph *graph,
		       ULONG numEgs, ULONG *egsVertexIndices)
{
  ULONG i;
  ...
  if (instanceList != NULL) {
	...
	egEndVertexIndex = egsVertexIndices[i + 1] - 1;
      else egEndVertexIndex = graph->numVertices - 1;
