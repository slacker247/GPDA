...
extern struct Air models[1000];
extern int      airlist;
extern int      n_aircraft;

static float    X, Y, Z, Azimuth;
extern int      Keyboard_mode;
int             _frame = 0;
extern float    Speed, DeltaSpeed, BaseSpeed;
   ...
   */
   for (i = 0; i < NUM_PLANES; i++) {
     ...
     */
     ...
     if (flock[i].PX < 1 || flock[i].PX > GRID_RANGE - 2 ||
	 flock[i].PY < 1 || flock[i].PY > GRID_RANGE - 2) flock[i].Pazimuth += M_PI;
