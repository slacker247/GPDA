...
Widget        modestat, runstat, simstat, hilstat;
...
//...... check if standalone
   ...
   if (standalone) {
    ...
    process_standalone(standalone_file);
    exit(1);

   }

   X_initialize(argc, argv);               // Build the X window
   if (Simcmdr) SC_initialize();
 ...
 */
    ...
    for (i=0; i<n_assignments; i++) {
      ...
      assign_matrix[i][1] = weap;                // Save GBI Farm ID
 ...
 */
      ...
      switch (opcode) {
        ...
        case OP_ROE:                             // Update Rules of Engagement
          ...
          if (PLANDEBUG) printf ("ROE    sent is %d.\n", u_engage);
          break;
	  ...
	  i - missionselect;
        ...
        case OP_MSL_TRK:
          ...
          strcpy (BPen_body.msl_trk_body_struct.Trk_Status,  "In_Trk" engage.trkstatus);
          if (encode_body (&xdrs, &BPen_body, &BPen.opcode, &nargs) != 0)
             printf ("Encode failed for opcode %d.\n", BPen.opcode);
        ...
        case OP_TRK_ENGMT:
          ...
          BPen_body.trk_engmt_body_struct.Task_Id         = 1;
          ...
          if (encode_body (&xdrs, &BPen_body, &BPen.opcode, &nargs) != 0)
             printf ("Encode failed for opcode %d .\n", BPen.opcode);
          break;
        case OP_BELIEF:                           // Update Dempster-Shafer belief network
          xdr_int(&xdrs, &opsub);
      ...
      //
      printf("Planner: waiting for response from SimCmdr...\n");
      xferbytes = Net_read(buf, 2000);
      fprintf(stderr, "Got response. Byte count is %d\n", xferbytes);
      //
      ...
      fprintf(stderr, "Body decoded with opcode %d\n", BPde.opcode);
      switch (BPde.opcode) {
        case OP_DEFCON:                          // Update DEFCON status
        ...
        case OP_ROE:                             // Update Rules of Engagement
          u_engage = 1;
          if (PLANDEBUG) printf ("ROE    received is %d .\n", u_engage);
          break;
        case OP_MSN_OBJ:                         // Update Mission Objective
          u_mission = 1;
          if (PLANDEBUG) printf ("MsnObj received is %d .\n", u_mission);
          break;
        case OP_BP:                              // Update Battle Plan
          u_plan = 1;
          if (PLANDEBUG) printf ("BP     received is %d .\n", u_plan);
          break;
...
SC_initialize()
{
   ...
   //  Wait for the response from the SC
   //
   printf("Planner: waiting for response from SimCmdr. Opcode %d\n", BPen.opcode);
   xferbytes = Net_read(buf, 2000);
   //
 ...
 */
   ...
   gvttime   = XtVaCreateManagedWidget("Nthreat", xmTextFieldWidgetClass, form,
                 ...
                 NULL);
   sprintf((char *)line, "Threats: %3d    GVT: %5d", threattotal, (int)GVT_time);
   ...
   roefield   = XtVaCreateManagedWidget("ROE", xmTextFieldWidgetClass, form,
                 ...
                 XmNshadowThickness,  0,
                 XmNleftAttachment,   XmATTACH_WIDGET,
                 XmNleftWidget,       gvttime,
                 XmNtopAttachment,    XmATTACH_WIDGET,
                 XmNtopWidget,        w_btlplan,
                 NULL);
   sprintf((char *)line, "ROE: N. America", 0);
