...
static long UseEntityTimeTag = 0L;
...
static VOID QueueObjectMatchAction(type,ins,slotNameID)
  ...
  int slotNameID;
  {
   OBJECT_MATCH_ACTION *prv,*cur,*new;
   ...
   new->type = type;
   new->nxt = cur;
   new->slotNameIDs = (type != OBJECT_MODIFY) ? NULL :
...
/****************************************************
  ...
  NOTES        : If the bitmap must be (re)allocated,
                 ...
                 to allow for growth.
 ****************************************************/
static SLOT_BITMAP *QueueModifySlotMap(oldMap,slotNameID)
  SLOT_BITMAP *oldMap;
  int slotNameID;
  {
   SLOT_BITMAP *newMap;
   ...
   if ((oldMap == NULL) ? CLIPS_TRUE : (slotNameID > (int)oldMap->maxid))
     {
      ...
      if (oldMap != NULL)
        {
         ...
         CopyMemory(char,oldsz,newMap,oldMap);
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
static VOID ReturnObjectMatchAction(omaPtr)
...
/******************************************************
  ...
  NOTES        : Incremental reset status is also
                 checked here
 ******************************************************/
static VOID MarkObjectPatternNetwork(slotNameIDs)
  SLOT_BITMAP *slotNameIDs;
  {
   OBJECT_ALPHA_NODE *alphaPtr;
   ...
   while (alphaPtr != NULL)
     {
      ...
      if ((id > (unsigned) clsset->maxid) ? CLIPS_FALSE : TestBitMap(clsset->map,id))
        {
         /* ===================================================
...
/***************************************************
  ...
  RETURNS      : CLIPS_TRUE if any common bits
                 are set in both maps, CLIPS_FALSE
                 otherwise
  SIDE EFFECTS : None
  NOTES        : None
 ***************************************************/
static BOOLEAN CompareSlotBitMaps(smap1,smap2)
  SLOT_BITMAP *smap1,*smap2;
  {
   unsigned short i,maxByte;
   
   maxByte = (unsigned short)
             ((int)((smap1->maxid < smap2->maxid) ? 
