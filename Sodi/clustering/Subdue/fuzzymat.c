...
DOUBLE FuzzyMatch( 
				  ...
				  DOUBLE maxMatchCost,			/* GV->matchCostMaxPercent * SizeOf(Subgraph2) for */
												/* AddVENoise() and AddENoise(); 
												   0.0 for GetStrongClasses() */
				  ...
				  ULONG initialMatchesCount,    /* 0 for AddVENoise() and AddENoise(); */
												/* 1 or 2 for extemp.c: GetStrongClasses() */
												...
												/* subGraph2->numberOfVertices; FALSE otherwise */
{
	...
	{
		if ( (expandingNode->matchCost < currentBestCost) || (currentBestCost < 0) )
		{
						 ...
						 LONG *matchingPattern )
{
	...
	if ( subGraph1->numberOfVertices >= subGraph2->numberOfVertices )
		matchCost =	FuzzyMatch( GV, subGraph1, subGraph1Reverse, subGraph2, 
		    subGraph2Reverse, maxMatchCost, 
			MaximumSearchNodes( subGraph1->numberOfVertices ),
			first, second, initialMatchesCount, matchingPattern, TRUE );
	else 
		matchCost =	FuzzyMatch( GV, subGraph2, subGraph2Reverse, subGraph1, 
		    subGraph1Reverse, maxMatchCost,
