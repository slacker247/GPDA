 ...
 **********************************************************************
 *
 * $Id: tools.cpp,v 1.3 2002/01/21 18:00:51 lurcher Exp $
 *
 * $Log: tools.cpp,v $
...
{
    QString autotest = test_list->text(test_list->currentItem());
...
void dRunAutoTests::add_auto_test( const char * test_name, MYQListViewItem *top )
{
    section *s = find_section( test_name );
        ...
        for ( prop *prop = s->first(); prop != 0; prop = s -> next())
        {
            if ( strcmp( prop -> name(), "DLL" ) == 0 )
            {
                ...
                if ( handle )
                {
                    ...
                    MYQListViewItem *item = new MYQListViewItem( top, test_name );  
