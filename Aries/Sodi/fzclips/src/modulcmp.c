...
/***********************************************************/ 
static VOID BeforeDefmodulesToCode()   /* added 03-08-96 */
  {
   int value = 0;
...
static int ConstructToCode(fileName,fileID,headerFP,imageID,maxIndices)
  ...
  int maxIndices;
  {
   ...
   if ((itemsFile = NewCFile(fileName,fileID,1,CLIPS_FALSE)) == NULL) 
     { return(CLIPS_FALSE); }
   ...
   if (portItemCount == 0) return(CLIPS_TRUE);
...
/************************************************************/
static int PortItemsToCode(fileName,fileID,headerFP,imageID,maxIndices,fileCount)
  char *fileName;
  int fileID;
  FILE *headerFP;
  int imageID;
...
/*********************************************************************/
static struct portItem *GetNextPortItem(theDefmodule,thePortItem,
                                        importChecked,exportChecked)
