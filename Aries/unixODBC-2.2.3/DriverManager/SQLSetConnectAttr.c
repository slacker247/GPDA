 ...
 **********************************************************************
 *
 * $Id: SQLSetConnectAttr.c,v 1.9 2002/07/24 08:49:52 lurcher Exp $
 *
...
static char const rcsid[]= "$RCSfile: SQLSetConnectAttr.c,v $ $Revision: 1.9 $";
           ...
           SQLINTEGER string_length )
{
    ...
    {
        if ((SQLINTEGER) value == SQL_OPT_TRACE_OFF )
        {
    ...
    {
        connection -> login_timeout = ( SQLINTEGER ) value;
    ...
    if ( connection -> state == STATE_C2 )
    {
        ...
        {
            connection -> connection_timeout = ( SQLINTEGER ) value;
            connection -> connection_timeout_set = 1;
        }
        else if ( attribute == SQL_ATTR_METADATA_ID )
        {
            connection -> metadata_id = ( SQLINTEGER ) value;
            connection -> metadata_id_set = 1;
        }
        else if ( attribute == SQL_ATTR_PACKET_SIZE )
        {
            connection -> packet_size = ( SQLINTEGER ) value;
        ...
        {
            connection -> txn_isolation = ( SQLINTEGER ) value;
