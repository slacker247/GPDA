...
GISP_Obj::pickEvent (GR_MouseEvent& event, GR_Window*)
{
   ...
   printf ("GISP object at lat = %f, lon = %f\n", get_lat()*180.0/M_PI, get_lon()*180.0/M_PI);
   switch (event.button)
   {
    case GR_LEFTMOUSE:
      if (event.down)
    ...
    case GR_RIGHTMOUSE:
      if (event.down)
      {
		 ...
		 FlatMap("pongyang.rgb", 0, get_lat()*180.0/M_PI, get_lon()*180.0/M_PI);
      }
      break;
   }
   printf("\n");
