...
extern void     PutOperator(int Severity, char *msg); 
   ...
   if (DSBMYSQL != 0) {
     ...
     if ((fp = fopen(chfile, "r")) != NULL) {
       ...
       icol = -1;
       irow = -1;
       fprintf(fp, "%4d   %-24s %8.3f  %8.3f %s %8.1f %c  %s  %s  %s  %s  %d  %d  %s\n", 
	   0,                      // etype
           chsource,               // hypothesis
           belief,                 // confidence
           nonbelief,              // disbelief
       ...
       sprintf(dsbtemp, "%s %s", "Sorry, specified story not defined - ", chmission);
       PutOperator(0, dsbtemp);
       return(-9);
     }
   }
...
//
   //
   ev.DS_loadEvidence(filename, DSBMYSQL,
...
float           T = 0.1;
   ...
   //
   sprintf(dsbtemp, "DSBFiles/%s.plan", "CND");
   if ((fp = fopen(dsbtemp, "r")) != NULL) {
