...
public class HTMLSummary extends LibraryItem {
  ...
  private void init(String filePath, boolean isIncluded) {
  ...
  public Collection createNewLibraryItems(Collection identifiers, boolean isIncluded, boolean isActive) {
    ...
    while(iter.hasNext()) {
      ...
      //item = new HTMLSummary(newFilePath, isIncluded);
      item = new HTMLSummary();
      item.init(newFilePath, isIncluded);
  ...
  public Collection getUserLibraryItems() {
    ...
    while(iter.hasNext()) {
      item = new HTMLSummary();
      item.init((String)iter.next(), false);
      items.add(item);
    }
    return items;
  }
  ...
  public Collection getAttributeValues(Object attributeName) {
    ...
    list.add("Info about the HTML doc could go here");
    return list;
  }
  ...
  public Collection query(String query, String queryTypeName, boolean showIncluded) {
    ...
    if(queryTypeName.equals(LibraryItem.FIND_ANY) || queryTypeName.equals("Links")) {
      ...
      while(iter.hasNext()) {
        ...
        for(i = 0; i <= link.length() - query.length(); i++) {
          if(link.regionMatches(true, i, query, 0, query.length())) {
            results.add(link);
          }
        }
      }
