...
GLUquadricObj   *quadobj;
   ...
   switch (sentype) {
     ...
     case 0:
       ...
       p_elev    = params[0];
       p_azi     = params[1];
       p_rmax_hi = params[2]/RE;
       p_rmin_hi = params[3]/RE;
       p_fov_hi  = params[4];
       ...
       newx = 1.0 + max/RE;                     // Take into account the length of the cone
       ...
       p_base = max*sin(fov/2.0*M_PI/180.0);
       //newx = max*sin((p_elev+(fov/2.0))*M_PI/180);
       ...
       gluCylinder(quadobj, (GLdouble)0.0, (GLdouble)p_base, (GLdouble)max, 32, 32);
