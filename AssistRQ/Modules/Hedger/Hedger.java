...
public class Hedger  extends AbstractLanguageAnalyser
  implements ProcessingResource {
   ...
   //Brings in the fields from the file hedges.rank and stores them in the class HedgeValue
   ...
   public void FileOutput(float avg)
   {
       ...
       try
       {
           Output_File = Output_File.substring(5);
           m_Output = new FileWriter(Output_File, true);
       }catch (IOException ioe)
       {
           Out.prln("IO error:" + ioe);
       }
...
// away it is from the hedge word.  Then whether it is a noun, pronoun, tags it.
           ...
           {
               nounTokenAnn = getclosestAnnot(nounTokenSet, hedgesAnnotsAnn, TokenSet);
           }
           ...
           if(negTokenSet != null)
           {
               ...
               negateTokenAnn = getclosestAnnot(negTokenSet, hedgesAnnotsAnn, TokenSet);
           }

           if(DEBUG.booleanValue()) Out.prln("Tagging sentence");

           if(nounTokenAnn != null)
           {
   ...
   public Annotation getclosestAnnot(AnnotationSet negTokenSet, Annotation hedgesAnnotsAnn, AnnotationSet TokenSet)
   {
      ...
      while(ntit.hasNext())
      {
         ...
         if(beforeAnnot != null)
         {
            if(negateTokenAnn.getStartNode().getOffset().longValue() > beforeAnnot.getStartNode().getOffset().longValue())
                test = true;
         }
         else
            test = true;
         if(negateTokenAnn.getStartNode().getOffset().longValue() < hedgesAnnotsAnn.getStartNode().getOffset().longValue() &&
               negateTokenAnn.getStartNode().getOffset().longValue() > Larger && test == true)
         {
         ...
         if(afterAnnot != null)
         {
            if((negateTokenAnn.getStartNode().getOffset().longValue() < afterAnnot.getStartNode().getOffset().longValue() &&
              negateTokenAnn.getStartNode().getOffset().longValue() > hedgesAnnotsAnn.getStartNode().getOffset().longValue()))
                test = true;
         }
         else
            test = true;
           if(negateTokenAnn.getStartNode().getOffset().longValue() > hedgesAnnotsAnn.getStartNode().getOffset().longValue() &&
               negateTokenAnn.getStartNode().getOffset().longValue() < Smaller && test == true)
         {
