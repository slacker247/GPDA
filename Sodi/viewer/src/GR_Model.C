...
GR_Model::delete_sobj (GR_Sensor* sobj)
{
  ...
  if (!DONE)
     printf ("Warning: deleting an inexistent sensor %d from platform %d.\007\n",
...
GR_Model::GR_Model (char* desc_file)
{
   ...
   char         *MODELDIR;
   char         *FWDIR;
   char		model_file [128], model_path[128];
...
//	 printf("... loaded.\n");
   ...
   /*
   char substring[80];
   printf(" %d Models processed.\n", p_files_read);
   for (i=0; i<p_files_read; i++) {
      strcat(strncpy(substring, p_filenames[i], strcspn(p_filenames[i],".")), "\0");
      printf(" Type %d   Name %s\n", p_filetypes[i], substring);
   }
   */
}

void
GR_Model::get_modelname(int type, int maxchars, char* name)
{
int    i, found, slen;

   found = FALSE;
   for (i=0; i<p_files_read; i++) 
      if (type == p_filetypes[i]) {
         found = i;
         break;
      }
   if (!found) {
     ...
     slen = strcspn(p_filenames[found],".");
     if (slen > maxchars-1) slen = maxchars-1;
     strcat(strncpy(name, p_filenames[found], slen), "\0");
   }
}
...
GR_Model::parse_model (char* filename)
{
   ...
   char		line [81];
   long		i, type;
   char		model_path[128], model_file [128];
   int          found;
   char         *MODELDIR;
   char         *FWDIR;
...
GR_Model::objdraw ()
{
  //printf ("doing GR_Model::objdraw %d %d (%d)\n", p_id, p_type, p_gr_objid);
...
get_scale (long type)
{
   ...
   switch (type)
   {
    ...
    case 34:
      ...
      break;
    case 37:
      sf = 0.025; // new_impact (was 0.011)
      break;
    case 38:
      sf = 0.025; //new_impact2 (was 0.01)
      break;
    ...
    case 101:
      ...
      break;
    case 102:
      sf = 0.16666666666666666; // awac_detected
      break;
    case 104:
