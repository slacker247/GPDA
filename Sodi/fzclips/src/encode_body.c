...
int encode_body (XDR *xdrs_en, struct SEND_ALL *en, int *opcode, int *nargs)
{
   ...
   switch (*opcode)
   {
	 ...
	 if (!xdr_int (xdrs_en, &(dp_all->intel_info_body_struct.II_Day)))
            {printf ("XDR_INT encode, Intel Info day of event, returns error.\n");}
         else
            {*nargs += 1;}

         if (!xdr_float (xdrs_en, &(dp_all->intel_info_body_struct.II_Hrs)))
            {printf ("XDR_FLOAT encode, Intel Info hours into the day of event, returns error.\n");}
         else
            {*nargs += 1;}

         if (!xdr_int (xdrs_en, &(dp_all->intel_info_body_struct.II_Nam_Num)))
            {printf ("XDR_INT encode, Intel Info number of sources being sent, returns error.\n");}
         else
            {*nargs += 1;}
 
	   for (i = 0; i < 5; i++)
	   {
             dpc = (char *) &(dp_all->intel_info_body_struct.II_Txt[i]);
             if (!xdr_string (xdrs_en, &dpc, fifty_two))
                {printf ("XDR_STRING encode, Intel Info message text, returns error.\n");}
             else
                {*nargs += 1;}
           }
	 
	 for (i = 0; i < dp_all->intel_info_body_struct.II_Nam_Num; i++)
	   {
	     dpc = (char *) &(dp_all->intel_info_body_struct.II_Nam_Src[i]);
             if (!xdr_string (xdrs_en, &dpc, four))
                {printf ("XDR_STRING encode, Intel Info source name, returns error.\n");}
             else
                {*nargs += 1;}

	      if (!xdr_int (xdrs_en, &(dp_all->intel_info_body_struct.II_Nam_Loc[i])))
                 {printf ("XDR_INT encode, Intel Info source number, returns error.\n");}
              else
                 {*nargs += 1;}

	      if (!xdr_float (xdrs_en, &(dp_all->intel_info_body_struct.II_Val[i])))
                 {printf ("XDR_FLOAT encode, Intel Info threat value, returns error.\n");}
              else
                 {*nargs += 1;}

	      if (!xdr_float (xdrs_en, &(dp_all->intel_info_body_struct.II_Per[i])))
                 {printf ("XDR_FLOAT encode, Intel Info persistence, returns error.\n");}
              else
                 {*nargs += 1;}
	   }
