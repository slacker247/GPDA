...
int             i, j;
   ...
   if(!fl_form_is_visible(fd_gagraph->gagraph) ) {
      fl_transient();
      fl_winposition(xpos, ypos);
      fl_initial_winsize(width, height);
      OPTwinid = fl_prepare_form_window(fd_gagraph->gagraph,
                    FL_PLACE_POSITION,FL_TRANSIENT, OPTlabel);
      ...
      fl_set_form_atclose(fd_gagraph->gagraph, OPTclose, 0);
      StoreActiveEntry(OPTlabel);
   }

   sprintf(opttemp, "%d", 10);
   fl_set_input(fd_gaparms->ga_geninc, opttemp);
   fl_set_counter_value(fd_gacommit->ga_goal, 11.0);
...
GAParameterList         params;
   ...
   theAppData.goal = fl_get_counter_value(fd_gacommit->ga_goal);
   theAppData.tolerance = fl_get_dial_value(fd_gacommit->opt_goodness)/100.0 * theAppData.goal;
...
float           tol;
   ...
   switch ((int)data) {
     ...
     case 3:
       tol = fl_get_dial_value(fd_gacommit->opt_goodness);
