      ...
      while (hashPtr != NULL)
         ...
         */   
         ...
         DeftemplateCConstructReference(fp, hashPtr->contents->whichDeftemplate,
                                        ImageID, MaxIndices);
         fprintf(fp,",");
         PrintSymbolReference(fp, hashPtr->contents->name);
...
/*****************************************************/
static int SymbolHashNodesToCode(fileName,version)
  char *fileName;
  int version;
  {
   int i, j;
...
/*****************************************************/
static int BitMapValuesToCode(fileName,version)
  char *fileName;
  int version;
  {
   int i, j, k, l;
...
/*   created using the constructs-to-c function.    */
/****************************************************/
static int FloatHashNodesToCode(fileName,version)
  char *fileName;
  int version;
  {
   int i, j;
...
/*   created using the constructs-to-c function.      */
/******************************************************/
static int IntegerHashNodesToCode(fileName,version)
  char *fileName;
  int version;
  {
   int i, j;
...
static int HashTablesToCode(fileName)
  char *fileName;
  {
   ...
   if ((fp = NewCFile(fileName,1,5,CLIPS_FALSE)) == NULL) return(0);
...
/*****************************************************/
globle VOID PrintSymbolReference(theFile,theSymbol)
  FILE *theFile;
...
/****************************************************/
globle VOID PrintFloatReference(theFile,theFloat)
  FILE *theFile;
...
/******************************************************/
globle VOID PrintIntegerReference(theFile,theInteger)
  FILE *theFile;
...
/*****************************************************/
globle VOID PrintBitMapReference(theFile,theBitMap)
  FILE *theFile;
...
/*********************************************************/
/* PrintCString: Prints CLIPS strings in the appropriate */
/*   format for C (the " and \ characters are preceeded  */
/*   by a \ and carriage returns are replaced with \n).  */
/*********************************************************/
static VOID PrintCString(theFile,str)
  FILE *theFile;
  char *str;
  {
   int i, slen;
