...
************************************************************************/
void
C_CONVERT::latlon_to_xyz(double lat, double lon, double x[3])
{
double re_cos_lat;
...
/************************************************************************
* lla_to_xyz : convert latitude, longitude, altitude to ECR xyz		*
************************************************************************/
void
C_CONVERT::lla_to_xyz(double lat, double lon, double alt, double x[3])
{
double re_cos_lat;
double sidereal_angle;

  re_cos_lat = (RE+alt) * cos(lat);
  sidereal_angle = lon;
...
************************************************************************/
void
C_CONVERT::latlon_to_xyz(double t, double lat, double lon, double x[3])
{
double re_cos_lat;
double sidereal_angle;
...
************************************************************************/
void
C_CONVERT::xyz_to_latlon(double x[3], double &lat, double &lon)
{
double xmag;
double rp;
  ...
  if (xmag > 50000.0) {
    fprintf(stderr, "XYZ2LL: Error, xmag too big\n");
    abort();
  }
  if (xmag < 6000.0) {
    fprintf(stderr, "XYZ2LL: Error, xmag too small %f %f %f\n",
	    x[0], x[1], x[2]);
    abort();
  }
  ...
  if (fabs(rp) < 0.001) {
    fprintf(stderr, "XYZ2LL: Error, hypot not working\n");
    rp = sqrt(x[0]*x[0] + x[1]*x[1]);
  }
  if (fabs(rp) < 0.001) {
    fprintf(stderr, "XYZ2LL: Error, input for xyz to latlon is bad\n");
    lat = PI/2;
  } else {
    lat = atan(x[2]/rp);
  }
...
************************************************************************/
void
C_CONVERT::xyz_to_latlon(double t, double x[3], double &lat, double &lon)
{
double rp;
...
************************************************************************/
void
C_CONVERT::eci_to_ecr(double t, double x[3], double v[3])
{
double co,so;
double x0, x1;
double v0, v1;
...
************************************************************************/
void
C_CONVERT::ecr_to_eci(double t, double x[3], double v[3])
{
double co,so;
double x0, x1;
double v0, v1;
