...
inline
time_t stringToTime(char *chtime)
{
int             year, month, day, hour, minute, second;
time_t          rettime;
struct tm       now;

   sscanf(chtime, "%d:%d:%d:%d:%d:%d",
...
inline
int timeToString(time_t *li, char *chtime)
{
  struct tm  *now;

  if (chtime == NULL)
  {
    fprintf(stderr, "NULL string parameter to timeToString\n");
    return -1;
  }

  now = localtime(li);
  sprintf(chtime, "%04d:%02d:%02d:%02d:%02d:%02d",
