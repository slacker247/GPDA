...
/**
 * This class knows how to create a DrawableGraph from a GML stream.
 *
 ...
 */
public class GMLInput {

  static Input _parser = new royere.cwi.input.GMLInput();

  /**
   * Parse GML character data form the given stream, create and return
   * a DrawableGraph
   *
   * @param in the input stream.
   * @param IOException if there is a stream error.
   * @param MalformedGMLException if the GML in the stream is bad.
   */
  public static DrawableGraph parseGML(InputStream in)
    throws IOException, MalformedGMLException
  {
    return (parseGML(in, null));
  }


  /**
   * In addition to parsing the GML stream, load any graphics (layout)
   * data into the given graph context object.
   *
   * @param in the input stream.
   * @param loader a LayoutLoader to load the layout.
   * @param IOException if there is a stream error.
   * @param MalformedGMLException if the GML in the stream is bad.
   */
  public static DrawableGraph parseGML(InputStream in,
    ...
    throws IOException, MalformedGMLException
  {
    ...
    // HACK ALERT!
    //
    // We are using the Royere GML parser until Ben slaps his
    // home-grown one in here.
    //
    // Also note, that Royere does not provide a way for me to handle
    // any syntax errors in the GML, so we never throw the
    // MalformedGMLException.
    //
    HashMap nodeMap = new HashMap();
    Graph g = _parser.parse(in);
    if (g == null) {
  ...
  public static boolean isLayoutSaved(DrawableGraph dg) {
    return ("true".equals(dg.getProperty(GMLOutput.P_LAYOUT_SAVED)));
  }
