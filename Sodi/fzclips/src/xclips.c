...
extern int CLIPSInputCount,EvaluatingTopLevelCommand;
...
/********** EXTERNAL FUNCTIONS **********/
extern int reroute_stdin();
extern int PrintCLIPS();
extern VOID ExitCLIPS();
extern int AddRouter();
extern int DeleteRouter();
extern int set_facts_watch();
extern int get_fact_watch();
extern int set_rules_watch();
extern int GetRulesWatch();
extern int set_activations_watch();
extern int get_activations_watch();
extern int set_execution_error();
extern int add_exec_function();
extern int remove_exec_function();
extern int dribble_active();
extern int expand_comand_string();
extern int AppendCommandString();
extern char *GetCommandString();
extern int (*SetEventFunction())();
extern int GetAgendaChanged();
extern VOID SetAgendaChanged();
extern VOID *GetNextActivation();
extern VOID SetFactListChanged();
extern VOID *GetNextInstance();
extern int GetFactListChanged();
extern VOID *GetNextFact();
extern VOID *GetNextDeffacts();
extern VOID *GetNextDeftemplate();
extern VOID *GetNextDefrule();
extern int GetFactDuplication();
extern int GetAutoFloatDividend();
extern int GetDynamicConstraintChecking();
extern int GetStaticConstraintChecking();
extern int GetIncrementalReset();
extern intGetResetGlobals();

/************ from xmenu.c *************/
extern void CreatePullDownMenus();
extern void MenuSelect();
extern void CreateAgendaWindow();
extern void CreateFactsWindow();
extern void CancelFileSelect();
extern Widget GetRuleManagerList();
...
void InitializeInterface()
{
  ...
  {
    PrintCLIPS("werror", "Could not initialize logical name hash table\n");
    XclipsExit(0);
    ExitCLIPS(0);
  }
...
/* Tell Clips which function to call periodically */
  AddPeriodicFunction("PeriodicUpdate",(int(*)())PeriodicUpdate,90);
...
char *log_name, *str;
{
  ...
  if(lptr == stdin_log)  /* standard IO : print to the main dialog */
    {
    str_list[0] = str;
    InsertClipsString(dialog_text, &event, str_list, &num);
    }
/* ============= */
/* agenda window */
/* ============= */
    ...
    {
    str_list[0] = str;
    InsertClipsString(agenda_text, &event, str_list, &num);
    }
...
/* ============= */
    ...
    {
    str_list[0] = str;
    InsertClipsString(facts_text, &event, str_list, &num);
    }
/* ================ */
/* instances window */
/* ================ */
   ...
   {
   str_list[0] = str;
   InsertClipsString(instances_text, &event, str_list, &num);  
   }
/* ================= */
/* defglobals window */
/* ================= */
   ...
   {
   str_list[0] = str;
   InsertClipsString(globals_text, &event, str_list, &num);
   }
/* ================= */
/* Focus window      */
/* ================= */
   ...
   {
   str_list[0] = str;
   InsertClipsString(focus_text,&event,str_list,&num);
   }
/* ==================== */
   ...
   while(instancePtr != NULL)
    {
       ...
       PrintCLIPS("xinstances",buffer);
       PrintCLIPS("xinstances","\n");
...
PrintChangedGlobals()
{
 ...
 /* Change the name of the window to the current module */
   ...
   while(dgPtr != NULL)
    {
      buffer = GetDefglobalPPForm(dgPtr);
      PrintCLIPS("xglobals",buffer);
      PrintCLIPS("xglobals","\n");
...
void UpdateMenus()
 ...
 if((Browse_status[FOCUS_WIN] == True) ? ( UpdateAll || (GetFocusChanged() == CLIPS_TRUE)) : FALSE)
     {
...
/*******************************************************************************
          ...
          Returns:       
*******************************************************************************/
static LogNamePtr LogInsert(logname)
  char *logname;
  {
  register int i;
  LogNamePtr node, ptr;
...
/*******************************************************************************
          ...
          Returns:       
*******************************************************************************/
static LogNamePtr LogLookup(logname)
  char *logname;
  {
   LogNamePtr ptr;
...
/*******************************************************************************
          ...
          Returns:     hvalue - 
*******************************************************************************/
static int GetHashValue(str, size)
  char *str;
  int size;
  {
  register unsigned i;
...
static int PeriodicUpdate()
{
 ...
 if((Browse_status[FOCUS_WIN] == True) ? (GetFocusChanged() == CLIPS_TRUE) :FALSE)
     {
      PrintChangedFocus();
      SetFocusChanged(CLIPS_FALSE);
     }
