...
public final class Parser implements ParserConstants {
    ...
    public void loadClses(KnowledgeBase kb, boolean isInclude, Collection errors) {
        try {
            ...
            Log.exception(e, this, "loadClses", kb, new Boolean(isInclude), errors);
            errors.add(e);
        }
    }
    ...
    private void checkUndefinedClasses() {
        if (!itsUndefinedClses.isEmpty()) {
            ...
            while (i.hasNext()) {
                Cls cls = (Cls) i.next();
                logError("Undefined class: " + cls.getName() + " found while " + state);
    ...
    private void checkUndefinedInstances() {
        if (!itsUndefinedInstances.isEmpty()) {
            ...
            while (i.hasNext()) {
                ...
                logError("Undefined instance: " + instance.getName());
                instance.setDirectType(cls);
            }
        }
    }

    public void loadInstances(KnowledgeBase kb, boolean isInclude, Collection errors) {
        try {
            ...
            Log.exception(e, this, "loadInstances", kb, new Boolean(isInclude), errors);
            errors.add(e);
        }
    }
    ...
    private static class ParseError extends Exception {
        ...
        public ParseError(String s, int lineNumber) {
            itsText = s;
            itsLine = lineNumber;
        }

        public String toString() {
            return "Error: " + itsText + " (line " + itsLine + ")";
        }
    }
    ...
    private void logError(String string) {
        // ParseError e = new ParseError(string, token.beginLine);
        // itsErrors.add(e);
        itsErrors.add("Error: " + string);
    }
    ...
    private void nameConflictError(String name, String typeName, Frame frame) {
        ...
        logError(text);
        Log.error(text, this, "nameConflictError");
    }
    ...
    private Cls getCls(String name, String comment, Collection parents, boolean isDefinition) {
        ...
        if (frame == null) {
            ...
            if ((++nClses % CLASS_TRACE_COUNT_DELTA) == 0 && nClses >= MIN_CLASS_TRACE_COUNT) {
                Log.trace("n=" + nClses, this, "getCls", name);
                Thread.yield();
            }
            if (comment != null) {
            ...
            if (isDefinition) {
                ...
                // Log.trace("remove undefined class", this, "getCls", name);
                if (!succeeded) {
                    logError("Duplicate definition of class " + name);
                }
            }
        } else {
    ...
    private Slot getExistingSlot(String name) {
        ...
        if (frame instanceof Slot) {
            ...
            logError("non-existent slot: " + name);
            slot = getSlot(name);
        }
        return slot;
    }
    ...
    private Instance getInstance(String instanceName, Cls cls) {
        Instance instance = itsKB.getInstance(instanceName);
        if (instance == null) {
            instance = itsKB.createInstance(instanceName, cls, false);
            if ((++nInstances % INSTANCE_TRACE_COUNT_DELTA) == 0 && nInstances >= MIN_INSTANCE_TRACE_COUNT) {
                Log.trace("n=" + nInstances, this, "getInstance", instanceName, cls);
                Thread.yield();
            }
            if (cls == null) {
        ...
        } else if (instance.getDirectType() != cls && cls != null) {
            if (isTypeMismatch(instance, cls)) {
                logError("type mismatch: " + instance + " cannot become an instance of " + cls);
            }
            Instance oldInstance = instance;
            instance = instance.setDirectType(cls);
            if ((++nInstances % INSTANCE_TRACE_COUNT_DELTA) == 0 && nInstances >= MIN_INSTANCE_TRACE_COUNT) {
                Log.trace("set type n=" + nInstances, this, "getInstance", instanceName, cls.getName());
                Thread.yield();
            }
    ...
    private static boolean isTypeMismatch(Instance instance, Cls cls) {
    ...
    private void tryCheckedValueAdd(Collection values, ValueType type, Object value) {
        if (type == ValueType.CLS || (type == ValueType.INSTANCE && !(value instanceof Instance))) {
    ...
    private Float toFloat(String s) {
        ...
        try {
            ...
            logError("Invalid floating point number: " + s);
            f = null;
        }
        return f;
    }
    ...
    private void addInverseSlotPair(Slot slot, Slot inverseSlot) {
        ...
        if (existingSlot == null) {
            ...
            logError("mismatched inverse slots: " + slot + ", " + inverseSlot + ", " + existingSlot);
        } else {
            // do nothing
        }
    }
    ...
    private void connectSuperslots(Slot slot, Collection superslots) {
        Slot directSubslots = itsKB.getSlot(Model.Slot.DIRECT_SUBSLOTS);
    ...
    private Facet getOrCreateFacet(String facetName) {
        ...
        if (facet == null) {
            facet = itsKB.createFacet(facetName);
        }
        return facet;
    }
...
// --------------------------------------------------------------------------------------
  ...
  final public void instance() throws ParseException {
    ...
    try {
      ...
      jj_consume_token(OF);
      type = clsReference();
            instance=getInstance(instanceName, type);
      label_12:
      while (true) {
        switch (jj_nt.kind) {
        case LPAREN:
  ...
  final public void slotValue(Instance instance) throws ParseException {
    ...
    jj_consume_token(RPAREN);
        if (slot == null) {
            logError("value for unknown slot: " + name + " in instance " + instance);
        } else {
  ...
  final private int[] jj_la1_0 = {0x0,0x3a80000,0x540000,0xffff0000,0x0,0xffff0000,0x0,0xffff0000,0x0,0x800,0xffff0000,0x10000,0xffff0000,0xffff0000,0x23a80000,0x200,0xffff0000,0xffff0000,0xffff0000,0x6bfe0000,0x8000,0x0,0x800,0xffff0400,0x8000,0x800,0x8000,0x800,0x0,0x0,0x0,0x8800,0x0,0xffff0000,0xffff0000,0xffff0000,0x80000000,0x0,0xffff0000,0x4000000,0x0,0xffff0000,};
  final private int[] jj_la1_1 = {0x18000100,0x0,0x0,0xffffffff,0x0,0xffffffff,0x0,0xffffffff,0x0,0x0,0xffffffff,0x1,0xffffffff,0xffffffff,0x1000000,0x14000000,0xffffffff,0xffffffff,0xffffffff,0x1580806,0x0,0x0,0x0,0xffffffff,0x0,0x0,0x0,0x0,0x2010000,0x2010000,0x8080,0x0,0x40004000,0xffffffff,0xffffffff,0xffffffff,0x10,0x80002000,0xffffffff,0x620,0xa00000,0xffffffff,};
  final private int[] jj_la1_2 = {0x0,0x0,0x0,0x4fff,0x2000,0x4fff,0x2000,0x4fff,0x2000,0x0,0x4fff,0x0,0x7fff,0x7fff,0x40,0x800,0x7fff,0x7fff,0x7fff,0x3d6,0x0,0x2000,0x0,0x4fff,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x1,0x4fff,0x7fff,0x7fff,0x0,0x0,0x4fff,0x28,0x0,0xfff,};
