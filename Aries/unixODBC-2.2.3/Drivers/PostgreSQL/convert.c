    ...
    if(fCType == SQL_C_CHAR) {
		...
		default:
			...
			if (cbValueMax > 0) {
				...
				case PG_TYPE_NUMERIC:
				{
					...
					/* Compute the string format for the string representation of the float/double/etc. value */
					total = strlen(ptr);
					dot = strstr(ptr, ".");
					if (dot)
						precision = total - (int)((dot + 1) - ptr);
					else
						precision = 0;
					sprintf(string_format, "%%%d.%df", total, precision);

					setlocale(LC_ALL, saved_locale);

					sprintf(new_string, string_format, d);
		...
		*/
		...
		case SQL_C_BINARY:	
			...
			len = convert_from_pgbinary(value, (unsigned char*)tempBuf, sizeof(tempBuf));
			ptr = tempBuf;
    ...
    for (opos = 0; opos < strlen(old_statement); opos++) {
		...
		*/
		...
		case SQL_VARBINARY:			/* non-ascii characters should be converted to octal */
			...
			npos += convert_to_pgbinary((unsigned char*)buf, &new_statement[npos], used);
...
unsigned int i, o = 0;
	...
	for (i = 0; i < strlen(in); i++) {
		if (in[i] == '+')
			...
			sprintf((char*)&out[o++], "%c", conv_from_hex((SQLCHAR*)&in[i]));
			i+=2;
		}
		else
			out[o++] = in[i];
