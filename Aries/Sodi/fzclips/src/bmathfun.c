...
/**********************************/
globle VOID AdditionFunction(returnValue)
  DATA_OBJECT_PTR returnValue; /* changed (result) 3-4-96 */
  {
   double ftotal = 0.0;
   long ltotal = 0L;
   BOOLEAN useFloatTotal = CLIPS_FALSE;
   EXPRESSION *theExpression; /* changed (test_ptr) 3-4-96 */
   DATA_OBJECT theArgument;   /* changed (rv) 3-4-96 */
   int pos = 1;
...
/*   routine for the * function.        */
/****************************************/
globle VOID MultiplicationFunction(returnValue)
...
/*   routine for the - function.     */
/*************************************/
globle VOID SubtractionFunction(returnValue)
  DATA_OBJECT_PTR returnValue;
  {
   double ftotal = 0.0;
   long ltotal = 0L;
...
/*   routine for the / function.   */
/***********************************/
globle VOID DivisionFunction(returnValue)
  DATA_OBJECT_PTR returnValue;
  {
   double ftotal = 1.0;
   long ltotal = 1L;
   ...
   while (theExpression != NULL)
     {
      ...
      if ((theArgument.type == INTEGER) ? (ValueToLong(theArgument.value) == 0L) :
                                 ((theArgument.type == FLOAT) ? ValueToDouble(theArgument.value) == 0.0 : CLIPS_FALSE))
        {
...
globle int SetAutoFloatDividendCommand()
  {
   ...
   RtnUnknown(1,&theArgument);  /* changed (&arg_ptr) 3-4-96 */
...
globle VOID MinFunction(returnValue)
  DATA_OBJECT_PTR returnValue;
  {
   DATA_OBJECT argValue;   /* changed (arg_value) 3-4-96 */
   int numberOfArguments, i;  /* changed (num_a) 3-4-96 */
   /*============================================*/
   /* Check for the correct number of arguments. */
   /*============================================*/
