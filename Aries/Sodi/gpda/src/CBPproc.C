...
FL_COLOR        STRATEGY_COLOR = FL_FREE_COL13;
FL_COLOR        THREAT_COLOR = FL_FREE_COL14;
FL_COLOR        ASSETS_COLOR = FL_FREE_COL15;
FL_COLOR        STRATEGY_LETTER_COLOR = FL_FREE_COL16;
...
int  CBPinit();
void CBPshow(int xpos, int ypos, int width, int height, Window winid, char *fn);
void CBPJcsExec();
...
/***
   fl_for_all_objects(fd_cbpinput->cbpinput,       applyResources, NULL);
   fl_for_all_objects(fd_caseinfo->caseinfo,       applyResources, NULL);
   fl_for_all_objects(fd_cbpview->cbpview,         applyResources, NULL);
   fl_for_all_objects(fd_cbpcoaview->cbpcoaview,   applyResources, NULL);
   fl_for_all_objects(fd_cbpmapview->cbpmapview,   applyResources, NULL);
   fl_for_all_objects(fd_cbprdeinput->cbprdeinput, applyResources, NULL);
   fl_for_all_objects(fd_cbprdehelp->cbprdehelp,   applyResources, NULL);
   fl_for_all_objects(fd_cbprdetext->cbprdetext,   applyResources, NULL);
***/

   strcpy(CBPlabel, "COA-Planning");

   fl_mapcolor(STRATEGY_COLOR, 182, 218, 202);
   fl_mapcolor(THREAT_COLOR, 225, 185, 186);
   fl_mapcolor(ASSETS_COLOR, 182, 217, 225);
   fl_mapcolor(STRATEGY_LETTER_COLOR, 0, 158, 12);
   fl_set_object_color(fd_cbpinput->goalsPanel, STRATEGY_COLOR, FL_COL1);
   fl_set_object_color(fd_cbpinput->cbp_cats[0], STRATEGY_COLOR, FL_COL1);
   fl_set_object_color(fd_cbpinput->threatsPanel, THREAT_COLOR, FL_COL1);
   fl_set_object_color(fd_cbpinput->cbp_cats[1], THREAT_COLOR, FL_COL1);
   fl_set_object_color(fd_cbpinput->assetsPanel, ASSETS_COLOR, FL_COL1);
   fl_set_object_color(fd_cbpinput->cbp_cats[2], ASSETS_COLOR, FL_COL1);

   //fl_hide_object(fd_cbpinput->cbp_save);
   fl_set_menu_item_mode(fd_cbpinput->cbp_parse, 1, FL_PUP_GREY);

   // Create the File menu with the first option having a submenu.
   fileMenu = fl_newpup(FL_ObjWin(fd_cbpinput->cbp_file));
   loadOrdersMenu = fl_newpup(0);
   fl_addtopup(fileMenu,"Load Order%m|Load Criteria%l|Clear|Save%l|Exit",
               loadOrdersMenu);
   fl_set_menu_popup(fd_cbpinput->cbp_file, fileMenu);

   // Create the View menu with the last option having a submenu.
   viewMenu = fl_newpup(FL_ObjWin(fd_cbpinput->cbp_view));
   optionsMenu = fl_newpup(0);
   fl_addtopup(viewMenu,"Order|COA|Map|Plan%l|Options%m", optionsMenu);
   fl_addtopup(optionsMenu, "Default Weighting%B%x11");
   fl_addtopup(optionsMenu, "Quick Search%B%x12");
   fl_addtopup(optionsMenu, "Optimize Search%B%x13");
   fl_addtopup(optionsMenu, "Repair Solution%B%x14");
   fl_addtopup(optionsMenu, "Manual Edit%B%x15");
   fl_addtopup(optionsMenu, "View%d%x16");
   fl_addtopup(optionsMenu, "Integrated%rg%x17");
   fl_addtopup(optionsMenu, "Defense%rg%x18");
   fl_addtopup(optionsMenu, "Offense%rg%x19");
   fl_set_menu_popup(fd_cbpinput->cbp_view, viewMenu);

   HideViewOptions();
   fl_setpup_mode(viewMenu, 1, FL_PUP_GREY);

   fl_set_counter_step(fd_cbpinput->cbpcases, 1.0, 1.0);
...
int CBPclose(FL_FORM *form, void *data)
{
   CBPexitCB(NULL, 0);
...
void CBPshow(int xpos, int ypos, int width, int height, Window mainwinID, char *fn)
{
int             i;
char            str[32];
   ...
   if(!fl_form_is_visible(fd_cbpinput->cbpinput) ) {
      ...
      CBPwinid = fl_prepare_form_window(fd_cbpinput->cbpinput,
                                     FL_PLACE_POSITION,FL_TRANSIENT, "Planning-Input");
      fl_winreshape(CBPwinid, xpos, ypos, width, height);
      fl_get_wingeometry(CBPwinid, &CBPwinX, &CBPwinY, &CBPwinW, &CBPwinH);
      CBPwinX = CBPwinX + 4;
      CBPwinY = CBPwinY + 24;
      fl_show_form_window(fd_cbpinput->cbpinput);
      fl_set_form_atclose(fd_cbpinput->cbpinput, CBPclose, 0);
      StoreActiveEntry("Planning-Input");

      fl_freepup(loadOrdersMenu);
      loadOrdersMenu = fl_newpup(0);
      fl_set_menu_popup(fd_cbpinput->cbp_file, fileMenu);

      //fl_addtopup(loadOrdersMenu, fn);
      if (strcmp(getenv("GPDADOMAIN"), "iow") == 0)
{
        fl_addtopup(loadOrdersMenu, "IDS-Attack-1234.5%x11");
printf("---adding (IDS-Attack-1234.5)\n");
}
      else {
        if (strlen(fn) < 28)
          strcpy(str, fn);
        ...
        strcat(str, "%x11");
        fl_addtopup(loadOrdersMenu, str);  //"JCS-Order-1437.5%x11");
printf("---adding fn -- (%s)\n", fn);
      }
fflush(stdout);
   }
...
void HideViewOptions()
{
  fl_setpup_mode(viewMenu, 2, FL_PUP_GREY);
  fl_setpup_mode(viewMenu, 3, FL_PUP_GREY);
  fl_setpup_mode(viewMenu, 4, FL_PUP_GREY);
}

void ShowViewOptions()
{
printf("IN ShowViewOptions\n");
fflush(stdout);
  fl_setpup_mode(viewMenu, 2, FL_PUP_NONE);
  fl_setpup_mode(viewMenu, 3, FL_PUP_NONE);
  fl_setpup_mode(viewMenu, 4, FL_PUP_NONE);
}
...
printf("CALLING ShowViewOptions\n");
fflush(stdout);
   ShowViewOptions();
}
...
char            chtemp[64];
FL_COLOR        CatColor[3] = { STRATEGY_LETTER_COLOR, FL_RED, FL_BLUE };
...
printf("-----IN jcsordersCB(%d)\n", item);fflush(stdout);
   switch (item) {
     case 0: // Process JCS Menu 
	  ...
	  //
          ShowKeys();
          fl_setpup_mode(viewMenu, 1, FL_PUP_NONE);
          fl_set_menu_item_mode(fd_cbpinput->cbp_parse, 1, FL_PUP_NONE);
       } else fl_show_messages("Error: Unable to open Cases file");
       break;

     case 1: // View JCS Order
       fl_set_form_position(fd_cbpview->cbpview, CBPwinX, CBPwinY);
       fl_set_form_size(fd_cbpview->cbpview, CBPwinW, CBPwinH); 
       ...
       sprintf(filename, "%s/%s", getenv("SEARCHDIR"), CaseFile);
       fl_load_browser(fd_cbpview->view_browser, filename);
       break;

     case 4: // Save case
       //sprintf(filename, "%s/%s.ctl", getenv("SEARCHDIR"), CaseFile);
	 ...
	 fl_set_object_color(fd_cbpcoaview->case_name[i], FL_INACTIVE_COL, FL_COL1);
	  ...
	  //
sprintf(chtmp, "%5.1f%%", (double)CaseMatch[caseno]);
   ...
   switch (item) {
	 ...
	 while (!feof(casefp)) {
	  ...
	  fprintf(stderr, "Unable to open Rules file: %s\n", filename);
       //
       fl_winposition(CBPwinX, CBPwinY);
       fl_initial_winsize(CBPwinW, CBPwinH);
...
Window          winid;
   ...
   switch (item) {
     case 0: /* handle the View Order screen */
       jcsordersCB(ob, 1);
       break;

     case 1: /* handle the COA Output screen */
       fl_set_input(fd_cbpcoaview->cbpcoa_caseid, CaseID);
     ...
     case 2: /* handle the COA Map screen */
       fl_set_input(fd_cbpmapview->cbpmap_caseid, CaseID);
...
void coafileCB(FL_OBJECT *ob, long data)
{
int  item;


   item = fl_get_menu(fd_cbpinput->cbp_file);
