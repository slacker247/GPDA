 ...
 */
public class TreeVis extends JPanel implements MouseMotionListener,
   ...
   */
  public TreeVis(TreeDisplayListener tdl, String dot, NodePlace p) {
   ...
   * @param p the algorithm to be used to position the nodes.
   */
  public TreeVis(TreeDisplayListener tdl, Node n, NodePlace p) {
    //if the size needs to be automatically alocated I will do it here
    setBorder(BorderFactory.createTitledBorder("Tree View"));
    m_listener = tdl;
    m_topNode = n;
...
/*******************************************************************************
   * To show a change in the tree from a previous run.
   */

  //To save the previous tree to a file for future compare.
  public void nodesToFile()
  {
    ...
    {
      Tree += m_nodes[i].m_node.getLabel() + "\n";
    }
    try
    {
      FileWriter m_Output = new FileWriter("Nodes.tmp", false);
      m_Output.write(Tree);
      m_Output.close();
    }catch(Exception ioe)
    {
  ...
  //To compare the tree from a File with the current tree.
  public void diffInNodes()
  {
    ...
    File f = new File("Nodes.tmp");
    if (f.exists())
    {
        try
        {
//            if (debug) System.out.println("The file in .rq : ");
            fis = new FileInputStream("Nodes.tmp");
...
//    if (debug) System.out.println("after br in export");
    ...
    //Extract the values line by line and checks them against the current tree
    while(sLine != null)
    {
        try
        {
...
//        if (debug) System.out.println("after node in export: " + sLine);
        ...
        if(x < m_numNodes)
        {
          if(!sLine.equalsIgnoreCase(m_nodes[x].m_node.getLabel()))
          {
            if((m_nodes[x].m_node.getChild(0)) == null)
            {
              changeNodeColor(x);
            }
          }
  ...
  //To change the color of the Node that has changed.
  public void changeNodeColor(int NodeValue)
  {
    Color color = new Color(255, 255, 0);
    m_nodes[NodeValue].m_node.setColor(color);
    System.out.println("Node Value: " + m_nodes[NodeValue].m_node.getLabel());
  }
    ...
    try
      {
        ...
        TreeVis a = new TreeVis(null, top, arrange);
        a.setSize(800 ,600);
        //a.setTree(top);
        JFrame f;
        f = new JFrame();
