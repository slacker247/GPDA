		      ...
		      Parameters *parameters)
{
  ...
  CopyUnmarkedGraph (graph, compressedGraph, vertexIndex);
		      ...
		      InstanceList *instanceList, ULONG overlapLabelIndex)
{
  ...
  while (instanceListNode1 != NULL) {
    ...
    for (v1 = 0; v1 < instance1->numVertices; v1++) {
      ...
      if (vertex1->used) { // (used==TRUE) indicates unchecked for sharing
	...
	while (instanceListNode2 != NULL) {
	  ...
	  for (v2 = 0; v2 < instance2->numVertices; v2++) {
	    ...
	    if (vertex1 == vertex2) { // point to same vertex, thus overlap
		...
		AddOverlapEdge (overlapEdges, & numOverlapEdges,
				...
				overlapLabelIndex);
	      // for external edges involving vertex1, duplicate for vertex2
	      for (e = 0; e < vertex1->numEdges; e++) {
		edge1 = & graph->edges[vertex1->edges[e]];
		if (! edge1->used) { // edge external to instance
		  overlapEdges =
		      ...
		      ULONG overlapLabelIndex)
{
  ...
  if (! found) {
    ...
    StoreEdge (overlapEdges, numOverlapEdges, sub1VertexIndex,
	       sub2VertexIndex, overlapLabelIndex, FALSE);
    numOverlapEdges++;
  }
			 ...
			 ULONG sub1VertexIndex, ULONG sub2VertexIndex)
{
  ...
  if (graph->vertices[edge->vertex1].map != sub1VertexIndex) {
    ...
    StoreEdge (overlapEdges, numOverlapEdges, v1, v2, edge->label,
	       edge->directed);
    numOverlapEdges++;
  } else { // edge connects Sub1 to another (or same) vertex in Sub1
			  ...
			  ULONG iteration, BOOLEAN predefinedSub)
{
  ...
  else sprintf (subLabelString, "%s_%lu", SUB_LABEL_STRING, iteration);
  ...
  if (allowInstanceOverlap &&
    ...
    else sprintf (overlapLabelString, "%s_%lu", OVERLAP_LABEL_STRING,
		  iteration);
  ...
  if (! predefinedSub) {
    ...
    // recompute graphs' MDL (always necessary since label list has changed)
				 ...
				 Parameters *parameters)
{
  ...
  for (e = 0; e < graph->numEdges; e++)
    graph->edges[e].label =
      StoreLabel (& labelList->labels[graph->edges[e].label],
		  newLabelList);
}
			     ...
			     Parameters *parameters)
{
  ...
  if (instanceList != NULL) {
	...
	instance = instanceListNode->instance;
	for (v = 0; v < instance->numVertices; v++) // subtract unique vertices
	  if (! graph->vertices[instance->vertices[v]].used) {
	    size--;
	    graph->vertices[instance->vertices[v]].used = TRUE;
	  }
	for (e = 0; e < instance->numEdges; e++) // substract unique edges
	  if (! graph->edges[instance->edges[e]].used) {
	    size--;
	    graph->edges[instance->edges[e]].used = TRUE;
	  }
...
void RemovePosEgsCovered (Substructure *sub, Parameters *parameters)
{
  ...
  for (posEg = 0; posEg < numPosEgs; posEg++) {
    ...
    else posEgEndVertexIndex = posGraph->numVertices - 1;
  ...
  CopyUnmarkedGraph (posGraph, newPosGraph, 0);
...
void CompressWithPredefinedSubs (Parameters *parameters)
{
  ...
  parameters->posGraphDL = MDL (posGraph, newLabelList->numLabels, parameters);
  if (parameters->negGraph != NULL)
    parameters->negGraphDL = MDL (negGraph, newLabelList->numLabels,
				  parameters);
}
