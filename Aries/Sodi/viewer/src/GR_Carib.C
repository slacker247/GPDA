...
Carib::draw_Carib (long lon, long lat)
{
   ...
   for (k=1, theta=theta_0; k<=6; k++, theta+= M_PI/360)
   {
      GR_bgnqstrip ();
      for (j=1, phi=phi_0; j<=4; j++, phi+= M_PI/360)
      {
	 vert[0] = (k - 1) / 6.0;
	 vert[1] = (j - 1) / 3.0;
	 // printf ("t=%f, s=%f\n", vert[0], vert[1]);
	 ...
	 // printf ("t=%f, s=%f\n", vert[0], vert[1]);
	 t2f (vert);
	 vert[0] = cos(phi) * sin (theta+M_PI/360) * factor;
	 vert[1] = sin(phi) * factor;
	 vert[2] = cos(phi) * cos (theta+M_PI/360) * factor;
	 GR_n3f (vert);
	 GR_v3f (vert);
      }
      GR_endqstrip ();
