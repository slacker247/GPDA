...
float   factor = 1.05;
   ...
   vertSW[0] = cos(phi1) * sin (theta0) * factor;
   vertSW[1] = sin(phi1) * factor;
   vertSW[2] = cos(phi1) * cos (theta0) * factor;
   vertNW[0] = cos(phi0) * sin (theta0) * factor;
   vertNW[1] = sin(phi0) * factor;
   vertNW[2] = cos(phi0) * cos (theta0) * factor;
   vertNE[0] = cos(phi0) * sin (theta1) * factor;
   vertNE[1] = sin(phi0) * factor;
   vertNE[2] = cos(phi0) * cos (theta1) * factor;
   vertSE[0] = cos(phi1) * sin (theta1) * factor;
   vertSE[1] = sin(phi1) * factor;
   vertSE[2] = cos(phi1) * cos (theta1) * factor;

   glBegin(GL_QUADS);
      glTexCoord2f(0, 0); glVertex3fv(vertSW);
      glTexCoord2f(0, 1); glVertex3fv(vertNW);
      glTexCoord2f(1, 1); glVertex3fv(vertNE);
      glTexCoord2f(1, 0); glVertex3fv(vertSE);
   glEnd();
   /*
