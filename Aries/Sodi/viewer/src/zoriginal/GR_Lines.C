...
GR_Lines::GR_Lines (short R, short G, short B, char* filename, long type)
{
   ...
   if (filename == NULL)
     p_filename = "/usr/esd/tung/GR/progs/World.bin";
   else
     p_filename = filename;
   ...
   for (i=0; p_polycount[i] != 0; i++)
   {
     ...
     {
        lon = p_polylines[i][j].lon;
        lat = p_polylines[i][j].lat;
	   ...
	   num_extra_pts = (int)(dist_sqr / threshold);
           if (num_extra_pts > 0)
           {
              printf ("... making %d extra points from old_lon=%f, old_lat=%f, to lon=%f, lat=%f ...\n",
                   num_extra_pts, old_lon*180/M_PI, old_lat*180/M_PI, lon*180/M_PI, lat*180/M_PI);
           }
	   ...
	   for (k=1; k<=num_extra_pts;k++)
	   {
		...
		old_lat * (num_extra_pts + 1.0 - k) / (num_extra_pts + 1.0);
	      ...
	      GR_v3f (vec);
	      printf ("   erxtra point at: lon=%f, lat=%f\n",
