...
/**
 * The class handles most of the management for a collection of frames.  It is a delegate of
 * the knowledge base class.  DefaultKnowledgeBase handles event notifications and journaling.
 * This class handles most other stuff.
 *
...
public class FrameManager implements Disposable {
    ...
    // <Reference, Object>
    public FrameManager(DefaultKnowledgeBase kb) {
        _knowledgeBase = kb;
    }
    ...
    public void addInstance(Instance instance, String name, Cls type) {
        ...
        /*
        // These are unnecessarily inefficient with the db backend
        setName(instance, name);
        setDirectType(instance, type);
        */
    ...
    public void addTemplateSlotValue(Cls cls, Slot slot, Object value) {
        _storage.addValue(cls, slot, null, true, value);
    }
    ...
    private void directRemoveInstanceOwnSlots(Cls cls, Slot slot) {
        Iterator j = getDirectInstances(cls).iterator();
        while (j.hasNext()) {
            Instance instance = (Instance) j.next();
            removeOwnSlot(instance, slot);
        }
    }
    ...
    public Collection getDirectTemplateFacetValues(Cls cls, Slot slot, Facet facet) {
    ...
    public Collection getDirectTemplateSlotValues(Cls cls, Slot slot) {
    ...
    private Collection getInheritedTemplateSlotValues(Cls cls, Slot slot) {
        Collection values = null;
        Iterator i = getDirectSuperclasses(cls).iterator();
        while (i.hasNext() & values != null) {
            Cls parent = (Cls) i.next();
            if (hasTemplateSlot(parent, slot)) {
                values = getTemplateSlotValues(parent, slot);
            }
        }
        return values;
    }
    ...
    public Collection getTemplateFacets(Cls cls, Slot slot) {
        ...
        while (i.hasNext()) {
            ...
            if (facet != null && facet.getAssociatedSlot() == ownSlot) {
                facets.add(facet);
            }
        }
        return facets;
    ...
    private boolean hasFrame(Frame frame) {
        return _storage.containsFrame(frame);
    }
     ...
     * if (hasDirectTemplateSlot(cls, slot)) {
     * isOverridden = overriddenRelativeToSlot(cls, slot);
     * } else {
     * isOverridden = overriddenRelativeToParents(cls, slot);
     * }
     * return isOverridden;
     * }
     * private boolean overriddenRelativeToSlot(Cls cls, Slot slot) {
     * boolean areDifferent = false;
     * Iterator i = getTemplateFacets(cls, slot).iterator();
     * while (i.hasNext() && !areDifferent) {
     * Facet facet = (Facet) i.next();
     * areDifferent = areDifferent(cls, slot, facet);
     * }
     * return areDifferent;
     * }
     * private boolean areDifferent(Cls cls, Slot slot, Facet facet) {
     * boolean areDifferent;
     * Collection clsValues = getDirectTemplateFacetValues(cls, slot, facet);
     * if (clsValues == null || clsValues.isEmpty()) {
     * areDifferent = false;
     * } else {
     * Collection slotValues = getOwnSlotValues(slot, facet.getAssociatedSlot());
     * areDifferent = !CollectionUtilities.equals(clsValues, slotValues);
     * }
     * return areDifferent;
     * }
     * private boolean overriddenRelativeToParents(Cls cls, Slot slot) {
     * boolean overridden = false;
     * Iterator i = getDirectSuperclasses(cls).iterator();
     * while (i.hasNext() && !overridden) {
     * Cls parent = (Cls) i.next();
     * if (hasTemplateSlot(parent, slot)) {
     * overridden = areDifferentBindings(parent, cls, slot);
     * }
     * }
     * return overridden;
     * }
     * private boolean areDifferentBindings(Cls parent, Cls child, Slot slot) {
     * boolean areDifferent = false;
     * Iterator i = getTemplateFacets(child, slot).iterator();
     * while (i.hasNext() && !areDifferent) {
     * Facet facet = (Facet) i.next();
     * areDifferent = areDifferent(parent, child, slot, facet);
     * }
     * return areDifferent;
     * }
     * private boolean areDifferent(Cls parent, Cls child, Slot slot, Facet facet) {
     * boolean areDifferent = false;
     * Collection childValues = getDirectTemplateFacetValues(child, slot, facet);
     * if (childValues == null || childValues.isEmpty()) {
     * areDifferent = false;
     * } else {
     * Collection parentValues = getTemplateFacetValues(parent, slot, facet);
     * areDifferent = !CollectionUtilities.equals(parentValues, childValues);
     * }
     * return areDifferent;
     * }
     */
    ...
    public boolean hasOverriddenTemplateFacet(Cls cls, Slot slot, Facet facet) {
        ...
        if (associatedSlot == null) {
            ...
            */
            ...
            result = !CollectionUtilities.equals(topLevelValues, values);
        }
        return result;
    }
    ...
    private boolean hasTemplateFacetValue(Frame frame, Slot slot, Facet facet, Object value) {
        return _storage.hasValue(frame, slot, facet, false, value);
    }
    ...
    public boolean hasType(Instance instance, Cls type) {
        ...
        if (directType == null) {
            ...
            hasType = directType == type || getSuperclasses(directType).contains(type);
        }
        return hasType;
    }
    ...
    private static boolean isModificationSlot(Slot slot) {
        ...
        return id == Model.Slot.ID.MODIFICATION_TIMESTAMP
            || id == Model.Slot.ID.MODIFIER
            || id == Model.Slot.ID.CREATOR
            || id == Model.Slot.ID.CREATION_TIMESTAMP;
    }
    ...
    private void propagateDefaultsToInstances(Cls cls, Slot slot, Collection defaults) {
        Iterator i = getInstances(cls).iterator();
        while (i.hasNext()) {
            Instance instance = (Instance) i.next();
            setOwnSlotValues(instance, slot, defaults);
        }
    }
    ...
    private void removeOwnSlot(Frame frame, Slot slot) {
        _storage.remove(frame, slot, null, false);
    }
