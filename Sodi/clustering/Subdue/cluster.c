...
PCLASSIFICATION_LATTICE CL_CreateLattice(char *latticeName)
{
	...
	for (i=0; i < labelStrLen; i++) {
		...
		labelStr[i] = (char)toupper(labelStr[i]);					// capitalize each letter
	}
...
PCLASSIFICATION_LATTICE CL_CreateNode(PGRAPH_VARIABLES GV, PSUB sub)
{
	...
	sprintf(labelStr, "%s%lu\0", SUB_LABEL_PREFIX, GV->subNumber, 
		sub->instances->currentLength );	// generating sub label
...
PCLASSIFICATION_LATTICE CL_Find(PCLASSIFICATION_LATTICE clRoot, const char *label)
{
	...
	for (i=0; i < clRoot->numberOfChildren; i++) {			// for all the children
		...
		if (CLNode)											// if child retured valid node
			  ...
			  ULONG height, BOOLEAN printOnScreen)
{
	...
	// write classification lattice hierarchy file, if subs are being saved
	if ( (clNode->referenceCount == 1) &&
		(clNode->numberOfChildren > 0) && 
		GV->saveBestSubDef && 
		GV->clHierarchyFile) 
	{
		fprintf(GV->clHierarchyFile, "%s : ", clNode->subLabel);
		for (i=0; i < clNode->numberOfChildren; i++) {		// for all the children
			fprintf(GV->clHierarchyFile, "%s ", clNode->children[i]->subLabel);
		}
		fprintf(GV->clHierarchyFile, " .\n");
	}
...
/*******************************************************************************
FUNCTION NAME: CL_TraverseInit
INPUTS:			PCLASSIFICATION_LATTICE clNode
RETURNS:		none
PURPOSE:		For all nodes in the CL set referenceCount = 0.
CALLED BY:		(unused)
*******************************************************************************/
...
#ifdef DEBUG_TRACE
	printf( "%s: CL_TraverseInit()\n", __FILE__ );
#endif

	clNode->referenceCount = 0;
	for (i=0; i < clNode->numberOfChildren; i++) {		// for all the children
		CL_TraverseInit(clNode->children[i]);			// init children recursively
	}
}
...
INPUTS:			PGRAPH_VARIABLES GV
				PCLASSIFICATION_LATTICE clNode
RETURNS:		none
PURPOSE:		Deallocate a CL recursively (or a CL node and all its children).
CALLED BY:		main.c: main()
*******************************************************************************/
void CL_Finish(PGRAPH_VARIABLES GV, PCLASSIFICATION_LATTICE clNode)
{
	...
	if (GV->cluster) {				// if clustering
		...
		GV->clHierarchyFile = fopen(CL_HIERARCHY_FILE, "w+");
		
		CL_Print(GV, clNode, 0, TRUE);			// printing lattice

		fprintf(GV->clusterFile, "}\n");		// close graph body
		fclose(GV->clusterFile);				// close cluster file
		fclose(GV->clHierarchyFile);			// close cluster file
