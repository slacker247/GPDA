...
C
      ...
      SPOS(2) = SenPos(2)
      SPOS(3) = SenPos(3)
      write(7,*) "Site= ", site(1), site(2), site(3),
     +           "  Sensor= ", spos(1), spos(2), spos(3)
...
C
      ...
      Q(2) = -T(1)*CSSLON*SNSLAT - T(2)*SNSLON*SNSLAT + T(3)*CSSLAT
      Q(3) =  T(1)*CSSLON*CSSLAT + T(2)*SNSLON*CSSLAT + T(3)*SNSLAT
C
...
C
      ...
      SINEL = Q(3) / QMAG
      EL = ASIN(SINEL)
      write(7,*) "  QMAG = ", Q(1), Q(2), Q(3), QMAG, SINEL
C
C        CALCULATE AZIMUTH
C
      AZ = DATAN2(Q(1),Q(2))
