...
GR_Terrain::GR_Terrain(long terid, long intype, float latitude, float longitude, char *terrafile)
{
...
C_CONVERT          conversion;
   ...
   if(intype == 2) {
      ...
      fd = fopen(chfile, "r");
      for (j=0; j<Iy; j++) {
         y[j] = (double)Elat - ((double)(j*Zskip)/120.0);
