...
/*******************************************************************************
FUNCTION NAME: SaveSub
INPUTS:		PGRAPH_VARIABLES GV
			PSUB_GRAPH sub
			PCHAR fileName
RETURNS:	NONE
PURPOSE:	Save the substructure into a file
...
void SaveSub( PGRAPH_VARIABLES GV, PSUB sub)
{
	...
	for (i=0; i < sub->instances->currentLength; i++)
	{
		sprintf(fileName, "%s%lu-%ld", 
			SUB_LABEL_PREFIX, 
			GV->subNumber,
			i);
		SaveInstance( GV, sub->instances->currentNode->subGraph, fileName );
	}
}
	
/*******************************************************************************
FUNCTION NAME: SaveInstance
INPUTS:		PGRAPH_VARIABLES GV
			PSUB_GRAPH sub
			PCHAR fileName
RETURNS:	NONE
...
void SaveInstance( PGRAPH_VARIABLES GV, PSUB_GRAPH sub, CHAR *fileName )
{
	...
	PLABEL label;
	ULONG *conversionTable;
	ULONG graphVertexIndex;

#ifdef DEBUG_TRACE
	printf( "%s: SaveInstance()\n", __FILE__ );
#endif
	
	file = fopen(fileName, "w+");
	if (file == NULL) {
		ErrorFatal("Error opening substructure file for writing.");
	}

	time( &ltime );
	fprintf(file, "%% Substructure file %s, created %s", fileName, ctime( &ltime ));

	conversionTable = (ULONG *)Calloc(GV->graph->numberOfVertices, sizeof(ULONG));
	...
	for (i=0; i < sub->numberOfVertices; i++) 
	{
		graphVertexIndex = sub->vertices[i].indexInGraph;
		conversionTable[graphVertexIndex] = i + 1;
		label = &(GV->labelList[graphVertices[graphVertexIndex].labelIndex]);
		fprintf(file, "v\t%d\t", i + 1 );
	...
	for (i=0; i < sub->numberOfVertices; i++) 
	{
		subVertex = &sub->vertices[i];
		for (j=0; j < subVertex->numberOfEdges; j++) 
		{
			graphVertex = &graphVertices[subVertex->indexInGraph];
			graphEdge = &graphVertex->edges[sub->edges[subVertex->edgesIndex + j].indexInGraphVertex];
			label = &(GV->labelList[graphEdge->labelIndex]);

			fprintf(file, "e\t%ld\t%ld\t", 
				conversionTable[subVertex->indexInGraph], 
				conversionTable[graphEdge->targetVertexIndex] );
