			 ...
			 ULONG numSubGraphs, Parameters *parameters)
{
  ...
  for (i = 0; i < numSubGraphs; i++) {
    ...
    for (v = 0; v < graph->numVertices; v++) {
      ...
      if (subNumber > 0) { // then valid reference to previous sub
			   ...
			   Graph *graph, LabelList *labelList, char *color)
{
  ...
  WriteLabelToFile (dotFile, graph->vertices[v].label, labelList);
			 ...
			 Graph *graph, LabelList *labelList, char *color)
{
  ...
  WriteLabelToFile (dotFile, edge->label, labelList);
  fprintf (dotFile, "\"");
  if (! edge->directed)
