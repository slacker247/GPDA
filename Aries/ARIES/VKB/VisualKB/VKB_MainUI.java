...
public class VKB_MainUI extends javax.swing.JFrame {
    ...
    public VKB_MainUI() {
        ...
        jBTN_ExecuteAssistRQ.setEnabled(true);
    ...
    */
	...
	try {
	    ...
	    evid[1] = ontMan.tagModeSI().getName();
	    RQform.setFieldValues(evid);
            
            Ontology ontology = ontMan.getOntology();
            String name;
        
            com.appliedminds.jam.AccessKBDlg bn = new com.appliedminds.jam.AccessKBDlg();
            // set the domain and story in the KB access object
            bn.Domain = this.g_Domain;
            bn.StoryName = this.g_Story;
        
            // get the drawable graph containing the nodes and edges
            com.appliedminds.martini.DrawableGraph graph = BNEView.getGraphPanel().getDrawableGraph();
        
            // get an interator on the nodes
            com.appliedminds.martini.NodeIterator iter = graph.nodesIterator();
        
            while(iter.hasNext()) {
                com.appliedminds.martini.DrawableNode node = iter.next();
                name = node.getProperty("label");
                for(int i = 0; i < ontology.depth; i++) {
                    for(int j = 0; j < ontology.levelElementNum[i]; j++) {
                        if(name.replace(' ', '_').equalsIgnoreCase(ontology.level[i][j].name))
                            node.setProperty("score", (new Integer(ontology.level[i][j].hits)).toString());
                    }
                }
            }
	...
	try {
		...
		Runtime.getRuntime().exec("java -cp .." + separator + "AssistRQ" +
                    pathSep + ".." + separator + "AssistRQ" + separator + "Gate" +
                    separator + "gate.jar" + pathSep + ".." + separator + "AssistRQ" +
                    separator + "Gate" + separator + "guk.jar" + pathSep + ".." +
                    separator + "AssistRQ" + separator + "Gate" + separator + "tools14.jar AssistRQ.AssistRQ ..");
	} catch(Exception ex) {
		ex.printStackTrace();
	}
    }
    ...
    private void initComponents() {//GEN-BEGIN:initComponents
        ...
        showClassifierCheckBox = new javax.swing.JCheckBoxMenuItem();
        ...
        showClassifierCheckBox.setText("Show Classifier");
        showClassifierCheckBox.setToolTipText("Show the semantic classifier values");
        showClassifierCheckBox.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                showClassifierCheckBoxActionPerformed(evt);
            }
        });

        jM_View.add(showClassifierCheckBox);
    ...
    /**
     * Show the bubbles displaying the semantic distance
     * classification information
     */
    private void showClassifierCheckBoxActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_showClassifierCheckBoxActionPerformed
        ...
        while(iter.hasNext()) {
            com.appliedminds.martini.DrawableNode node = iter.next();
            if(showClassifierCheckBox.isSelected())
                node.setProperty("scoreBubble", "true");
            else
                node.setProperty("scoreBubble", "false");
            //node.setProperty("score", "34");
        }
        
        // refresh the graph
        BNEView.getGraphPanel().redraw();
    }//GEN-LAST:event_showClassifierCheckBoxActionPerformed
    ...
    private javax.swing.JCheckBoxMenuItem showClassifierCheckBox;
