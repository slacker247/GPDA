								...
								LPDWORD	pnUsageCount )
{
	...
	if ( iniPropertySeek( hODBCInstIni, (char *)pszDriver, "FileUsage", "" ) == INI_SUCCESS )
	{
	...
	if ( iniObjectSeek( hODBCInstIni, (char *)pszDriver ) == INI_SUCCESS )
	{
		...
		else
		{
			if ( iniPropertySeek( hODBCInstIni, (char *)pszDriver, "FileUsage", "" ) == INI_SUCCESS )
			{
				...
				iniPropertyUpdate( hODBCInstIni, "FileUsage", szValue );
			}
			else
			{
				iniPropertyInsert( hODBCInstIni, "FileUsage", szValue );
			}
		}
