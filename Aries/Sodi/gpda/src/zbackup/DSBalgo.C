...
int Transfer_Data (void)
{
  ...
  double    fused[3];          // Data fused together
  double    in[3];             // New data
  double    sendTime;          // Message send time
   ...
   * child, the result stored in the 'In' variable.  This non-fused       *
   * data is next fused with the fused weighted data from any previously  *
   * processed parent nodes, which resides in the 'fused' variable,       *
   * and the result is stored in the 'fused' variable.  After all         *
    ...
    if (nodes[chIndex].needsUpdate)
    {
      ...
      for (parIndex = 0; parIndex < DS_numNodes; parIndex++)
      {
            ...
            (DS_links[D][chIndex][parIndex] != 0.0))
        {
           ...
           * impact weights and store in the 'in' variable. */
          ...
          for (bd = B; bd <= D; bd++)
          {
            in[bd] = DS_links[bd][chIndex][parIndex] *
              DS_nodeValues[bd][parIndex];
          }
          ...
          if (doingFirstParent)
          {
            ...
            sendTime = nodes[parIndex].EpocSecSnd;
          ...
          else  /* Fuse new weighted inputs with previously fused data. */
          {
            ...
            Fuse_BUD
              (fused[B], fused[D],
               ...
               fused[B], fused[U], fused[D]);
            
            if (sendTime < nodes[parIndex].EpocSecSnd)
            {
              sendTime = nodes[parIndex].EpocSecSnd;
            }
          }
      ...
      /* Copy fused data to the child node's In field. *
       * Enter time & other values to node cell.       */

      for (bd = B; bd <= U; bd++)
        nodes[chIndex].In[bd] = fused[bd];

      nodes[chIndex].EpocSecRcv = 0.0;

      if (nodes[chIndex].EpocSecSnd < sendTime)
      {
        nodes[chIndex].EpocSecSnd = sendTime;
      }
...
int Fuse_Cells(int nodeIndex, short Is_External_Input, short &Have_Evid)
{
  ...
  if (nodes[nodeIndex].needsUpdate)
  {
    if (Is_External_Input)
    {
      ...
      if ((fabs(DS_nodeValues[B][nodeIndex]) < EPSLON) &&
          (fabs(DS_nodeValues[D][nodeIndex]) < EPSLON))
      { 
        /* Node's first input, just copy new input to fused. */
        for (bd = B; bd <= U; bd++)
          DS_nodeValues[bd][nodeIndex] = nodes[nodeIndex].In[bd];
      }
      else
      { 
        ...
        Fuse_BUD (DS_nodeValues[B][nodeIndex],
                  DS_nodeValues[D][nodeIndex],
                  nodes[nodeIndex].In[B],
                  nodes[nodeIndex].In[D],
                  nodes[nodeIndex].Cell_Name,
                  DS_nodeValues[B][nodeIndex],
                  DS_nodeValues[U][nodeIndex],
                  DS_nodeValues[D][nodeIndex]);
      }
    ...
    if ((DS_nodeValues[B][nodeIndex] > 1.0) || 
        (DS_nodeValues[B][nodeIndex] < 0.0) || 
        (DS_nodeValues[D][nodeIndex] > 1.0) || 
        (DS_nodeValues[D][nodeIndex] < 0.0))
    {
      fprintf(stderr,
    ...
    else if (DS_nodeValues[B][nodeIndex] + DS_nodeValues[D][nodeIndex] > 1.0)
    {
      fprintf(stderr,
...
int DS_AlgoInit(char *missionName, char* storyName, char* cfgName,
                int verbose, int readFromDB)
{
  ...
  returnVal = DS_InitAttributes(missionName, storyName, verbose, readFromDB);
  DS_AlgoInitConfig();
  ret = DS_AlgoReadConfig(cfgName, TRUE);
...
void Reset_Node_Values(void)
{
  for (int node = 0; node < DS_numNodes; node++)
  {
    ...
    DS_nodeValues[B][node]      = 0.0;
    DS_nodeValues[U][node]      = 1.0;
    DS_nodeValues[D][node]      = 0.0;
...
void DS_AlgoPrintTree(void)
{
  ...
  for (level = 0; level < DS_treeDepth; level++)
  {
    for (bd = B; bd <= D; bd++)
    {
      ...
      {
        printf ("  % 8.6f", DS_nodeValues[bd][DS_CoordToIndex(node,level)]);
      }
...
int DS_AlgoGetResult(int node, int level, float results[])
{
  ...
  results[1] = DS_nodeValues[B][nodeIndex];
  results[2] = DS_nodeValues[D][nodeIndex];
  results[3] = DS_nodeValues[U][nodeIndex];
...
int DS_AlgoGetInverse(int node, int level, float results[])
{
  ...
  A[B] = DS_nodeValues[B][index];
  A[D] = DS_nodeValues[D][index];
  A[U] = DS_nodeValues[U][index];
  adjust(&A[B], &A[U], &A[D]);
  //printf("\n\nA    = [%5.3f  %5.3f  %5.3f]\n", A[B], A[U], A[D]);
...
void DS_AlgoSetNumFusions (int num)
{
  numFusions = num;
}
...
int DS_AlgoOverrideNodeValues (int node, int level, float New_B, float New_D)
{
  if ((level < 0) || (level >= DS_treeDepth))
  {
    printf ("\n\nDS_AlgoOverrideNodeValues:  Level %d out of range, "
            "no changes made.\n\n", level);
    return -1;
  }
  ...
  {
    printf ("\n\nDS_AlgoOverrideNodeValues:  node %d out of range, "
            "no changes made.\n\n", node);
    return -1;
  }
  ...
  {
    printf ("\n\nDS_AlgoOverrideNodeValues:  New belief %5.3f and/or disbelief"
...
void Compute_Delta(int par, int bd, short changeAnyLinks)
{
  ...
  actualOutput = DS_nodeValues[bd][par];
...
int DS_AlgoPropogateOverrides (int Constraint_Level, float Learning_Rate,
                               int Max_Backpropogation_Levels)
{
  ...
  {
    printf("DS_AlgoPropogateOverrides : Constraint level %d is out of range, "
  ...
  {
    printf("DS_AlgoPropogateOverrides : Learning rate %5.3f is out of range, "
           "resetting it to 0.0\n", Learning_Rate);
    Learning_Rate = 0.0;
    returnValue = -1;
  }
  ...
  {
    printf("DS_AlgoPropogateOverrides : Max backpropogation levels %d is out "
    ...
    for (i = 0; i < DS_numNodes; i++)
    {
          ...
          (nodes[i].Desired[D] != -1.0))
      {
        printf ("\nOld %1d node values:  B = %f  D = %f",
                nodes[i].Cell_Name, DS_nodeValues[B][i], DS_nodeValues[D][i]);
      }
    }
...
fflush(stdout);
  ...
  for (bd = B; bd <= D; bd++)
  {
    ...
    for (i = 0; i < DS_numNodes; i++)
    {
      if (nodes[i].Desired[bd] != -1.0)
      {
        error = fabs (DS_nodeValues[bd][i] - nodes[i].Desired[bd]);
        ...
        for (par = 0; par < DS_numNodes; par++)
        {
              ...
              (Change_Any_Links && (nodes[par].level == nodes[ch].level - 1)))
          {
            New_Impact = DS_links[bd][ch][par] +
              Learning_Rate * nodes[ch].delta[bd] * DS_nodeValues[bd][par];
...
// This code allows negative weights:
                ...
                (DS_links[bd][ch][par] != New_Impact))
            {
              printf("\n%c Impact [%d][%d] = %f + %f * %f * %f = %f\n",
                     ...
                     nodes[ch].delta[bd], DS_nodeValues[bd][par], New_Impact);
              fflush(stdout);
            }
