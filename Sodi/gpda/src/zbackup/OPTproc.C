...
GAGenome        *best;
  ...
  fl_addto_browser(fd_gagraph->gadata_browser, bigstring);
   ...
   sprintf(opttemp, " No. Response Options %5d\n\n", theAppData.lengthGenome/2);
   fl_addto_browser(fd_gagraph->geneview, opttemp);

   for(int i=0; i<theAppData.lengthGenome; i=i+2) {
     sprintf(opttemp, "RO(%2d) = %-16s\nTgt(%2d) = %-16s\n",
             i/2, options[(int)(((GARealGenome&)(ga->population().best())).gene(i))].Option,
             i/2, alltgts[(int)(((GARealGenome&)(ga->population().best())).gene(i+1))].Name );
     fl_addto_browser(fd_gagraph->geneview, opttemp);
   }
