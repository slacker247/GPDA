...
//                              valid only of GraphMatch returns TRUE
		    ...
		    double threshold, double *matchCost, VertexMap *mapping)
{
  ...
  else cost = InexactGraphMatch(g1, g2, labelList, threshold, mapping);
			  ...
			  double threshold, VertexMap *mapping)
{
  ...
  while ((! MatchHeapEmpty (globalQueue)) && (! done)) {
    ...
    if (node.cost < bestNode.cost) {
      if (node.depth == nv1) {	// complete mapping found
	...
	if (! quickMatch)
      ...
      } else { // expand node's partial mapping
	...
	} else MergeMatchHeaps (localQueue, globalQueue); // clears localQueue
      }
    } else free (node.mapping);
			 ...
			 ULONG *mapped1, LabelList *labelList)
{
  ...
  for (e1 = 0; e1 < g1->vertices[v1].numEdges; e1++) {
    ...
    else otherVertex1 = edge1->vertex1;
	...
	if ((! edge2->used) &&
	  ...
	  if (edge1->directed && edge2->directed &&
	      ...
	      (((edge1->vertex1 == v1) && (edge2->vertex1 == otherVertex2)) ||
	       ((edge1->vertex1 == otherVertex1) && (edge2->vertex1 == v2))))
...
double InsertedVerticesCost (Graph *g2, ULONG *mapped2)
{
  ...
  for (v = 0; v < g2->numVertices; v++)
	...
	else v2 = g2->edges[g2->vertices[v].edges[e]].vertex1;
