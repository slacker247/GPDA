...
void translator (char *messages, char *decisions, unsigned int *xdrsize)
{
   ...
   int                  nargs_in, nargs_out, i, j, call_SC, test = 0, num;
char  chsource[5][16] = { "Unknown", "Open Source", "Human Intel",
			               "Signal Intel", "Image Intel" };
char  ipline[1280];	
char  chsrc[12];

   unsigned int		four = 4, eight = 8, twelve = 12, twenty = 20, fifty_two = 52;
...
/*Prepare message body for SC. */
 ...
 /*  Call Sim Cmdr here.  */
	...
	DirectPutSlot (msg_body, "gbi_task_id", &result);
	 ...
	 if (!xdr_int (&xdrs, &SC_intel_info_body.II_Day))
            printf ("XDR_INT decode, Intel Info day of event, returns error.\n");
         else
            {nargs_out += 1;
             if (toprttr == 1)
                printf ("Intel Info day of event is %d .\n", SC_intel_info_body.II_Day );}
	 
	 if (!xdr_float (&xdrs, &SC_intel_info_body.II_Hrs))
            printf ("XDR_FLOAT decode, Intel Info Hrs, returns error.\n");
         else
            {nargs_out += 1;
             if (toprttr == 1)
                printf ("Intel Info hours into day of event is %f .\n", SC_intel_info_body.II_Hrs);}

	 if (!xdr_int (&xdrs, &SC_intel_info_body.II_Nam_Num))
            printf ("XDR_INT decode, Intel Info number of sources, returns error.\n");
         else
            {nargs_out += 1;
             if (toprttr == 1)
                printf ("Intel Info number of sources is %d .\n", SC_intel_info_body.II_Nam_Num );}
	 
	 dp_intel_info_body = &SC_intel_info_body;
  
       	 for (i = 0; i < 5; i++)
	   {
	      dp          = dp_intel_info_body->II_Txt[i];
	      if (!xdr_string (&xdrs, &dp, fifty_two))
		printf ("XDR_STRING decode, Intel Info message text, returns error.\n");
	      else
		{nargs_out += 1;
		if (toprttr == 1)
		  printf ("Intel Info message text is %s .\n", SC_intel_info_body.II_Txt[i] );}
	   }
	 
	 for (i = 0; i < SC_intel_info_body.II_Nam_Num; i++)
	   {
	      dp          = dp_intel_info_body->II_Nam_Src[i];
	      if (!xdr_string (&xdrs, &dp, four))
		printf ("XDR_STRING decode, Intel Info source name, returns error.\n");
	      else
		{nargs_out += 1;
		if (toprttr == 1)
		  printf ("Intel Info source name is %s .\n", SC_intel_info_body.II_Nam_Src[i] );}
	 
	       if (!xdr_int (&xdrs, &SC_intel_info_body.II_Nam_Loc[i]))
		 printf ("XDR_INT decode, Intel Info source type location, returns error.\n");
	       else
		 {nargs_out += 1;
		 if (toprttr == 1)
		   printf ("Intel Info source type location is %d .\n", SC_intel_info_body.II_Nam_Loc[i] );}
	
	        if (!xdr_float (&xdrs, &SC_intel_info_body.II_Val[i]))
		  printf ("XDR_FLOAT decode, Intel Info threat value, returns error.\n");
		else
		  {nargs_out += 1;
		  if (toprttr == 1)
		    printf ("Intel Info threat value is %f .\n", SC_intel_info_body.II_Val[i]);}

		 if (!xdr_float (&xdrs, &SC_intel_info_body.II_Per[i]))
		   printf ("XDR_FLOAT decode, Intel Info persistence, returns error.\n");
		 else
		   {nargs_out += 1;
		   if (toprttr == 1)
		     printf ("Intel Info persistence is %f .\n", SC_intel_info_body.II_Per[i]);}
	   }
	 ...
	 result.type = INTEGER;
	 result.value = AddLong (SC_intel_info_body.II_Day);
	 DirectPutSlot (msg_body, "day" , &result); 
	 
	 
	 result.type = FLOAT;
	 result.value = AddDouble (SC_intel_info_body.II_Hrs);
	 DirectPutSlot (msg_body,"time" , &result); 

	 SC_intel_time (SC_intel_info_body.II_Day, SC_intel_info_body.II_Hrs);
	...
	for (i = 0; i < SC_intel_info_body.II_Nam_Num; i++)
	  {  
	    ...
	    result.type = STRING;
	    result.value = AddSymbol (SC_intel_info_body.II_Nam_Src[i]);
	    DirectPutSlot (info, "source_type", &result); 
	 
	    result.type = INTEGER;
	    result.value = AddLong (SC_intel_info_body.II_Nam_Loc[i]);
	    DirectPutSlot (info, "source_num", &result); 

	    result.type = FLOAT;
	    result.value = AddDouble (SC_intel_info_body.II_Val[i]);
	    DirectPutSlot (info, "value", &result); 

	    result.type = FLOAT;
	    result.value = AddDouble (SC_intel_info_body.II_Per[i]);
	...
	for (i=0; i<SC_intel_info_body.II_Nam_Num; i++) {
	   j = 0;
	   strcpy(chsrc, SC_intel_info_body.II_Nam_Src[i]);
       if (chsrc[0] == 'O') j = 1;
	...
	sprintf (line, "%s\n%s\n%s\n%s\n%s\n", SC_intel_info_body.II_Txt[0], SC_intel_info_body.II_Txt[1], 
		 SC_intel_info_body.II_Txt[2], SC_intel_info_body.II_Txt[3], SC_intel_info_body.II_Txt[4]);
	SC_intel_text (line);
...
/*Feed the translator the return message from FZCLIPS  */
  ...
  /*Get the destination  id . Upon return this will be the id of a Battle Planner. */
   ...
   if (Dummy_Opcode == OP_EXIT_SC)
    
      printf ("***SC has exited.***\n");
