...
//        printf("GR_DispList: drawing %d objects\n", p_total_objects);
	for (i=0; i < p_total_objects; i++)
        {
           if ( (GR_pickflag == 0) ||
                (GR_pickflag == 1 && 
...
//	   printf("Type and ID is %d  %d\n", p_object_list[i]->get_type(),
//					     p_object_list[i]->get_objid() );
        }
}
...
void
GR_DispList::delete_track (long type)
{
        int   i, j;

	i = 0;
        //for (i=0; i<p_total_objects; i++)
	while (i<p_total_objects)
        {
                if (p_object_list[i]->get_trackid() == type)
                {
                        for (j = i+1; j < p_total_objects; j++)
                                p_object_list [j-1] = p_object_list [j];
