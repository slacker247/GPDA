...
extern int DS_AlgoInit
  (char  *missionName,     // In - Name of DB or attribute file to be read
   char  *storyName,       // In - Name of story
   char  *cfgName,         // In - Name of config file
  ...
  // missionName = NULL to create a new tree that will be defined by calls
  // to the DS_Set... functions.  Set readFromDB to TRUE to read data from
  // the storyName table of the missionName database, or to FALSE to read
  // data from the missionName falt file.  Returns 0 on success, -1 on error,
  // or -2 if DB option is requested before DB it has been implemented.
...
extern int DS_AlgoGetNumFusions (void);

  // DS_AlgoGetNumFusions returns the number of fusions that have occured
  // since DS_AlgoInit or DS_AlgoReset or DS_AlgoSetNumFusions was called.


extern void DS_AlgoSetNumFusions (int num);

  // DS_AlgoSetNumFusions resets the fusion counter to the parameter value. 


extern int DS_AlgoOverrideNodeValues
  ...
  // DS_AlgoOverrideNodeValues sets the specified node's Desired-Belief
  // and -Disbelief to the parameter values.  Subsequent running of
  // DS_AlgoPropogateOverrides will alter various tree link weights to
...
extern int DS_AlgoPropogateOverrides
  ...
  // DS_AlgoPropogateOverrides allows the operator to override node belief
  ...
  // (via the DS_AlgoOverrideNodeValues function).  On error (such as
