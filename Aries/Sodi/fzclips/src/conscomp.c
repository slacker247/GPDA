...
#endif
#define SECONDARY_CODES "ABCDEFGHIJKLMNOPQRSTUVWXYZ" /* 03-04-96 */
#define SECONDARY_LEN   26  /* changed (36)  03-04-96 */
...
#if FUZZY_DEFTEMPLATES  /* added 03-04-96 */
   fprintf(HeaderFP,"#include \"fuzzylv.h\"\n");
   fprintf(HeaderFP,"#include \"fuzzyval.h\"\n");
#endif
   fprintf(HeaderFP,"\n#define VS (VOID *)\n");
   fprintf(HeaderFP,"\n");
...
static VOID WriteFunctionExternDeclarations(fp)
  FILE *fp;
  {
   struct FunctionDefinition *theFunction; /* changed 03-04-96 */
...
#if FUZZY_DEFTEMPLATES
   fprintf(fp,"   SetFuzzyValueTable(fvht%d);\n",ImageID);
#endif
...
#if SHORT_LINK_NAMES
      if ((unsigned int)strlen(arrayName) > 6)
        {
