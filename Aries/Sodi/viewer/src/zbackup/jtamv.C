...
New things for Version 0.7a:
	...
	12) Picking of sensor or trail displays status dialog
	13) User defined views on 'View' menu using 'viewmenu.dat'
...
#ifdef Linux
extern "C" void f90_init(int argc, char *argv[]);
extern "C" void f90_finish(int status);
#endif
...
char * str;
   ...
   printf("\n\n");
   printf("  -- JNTF Theater Air and Missile Visualizer (JTAMV) --\n");
   printf("                   (Version 0.7a)");
   printf("\n\n");
...
*/
   ...
   if ((menufile = fopen("viewmenu.dat", "r")) != NULL) {
      fscanf(menufile, "%d\n", &n_items);
      for (i=0; i<n_items; i++) {
...
weatherCB (Widget, XtPointer client_data, XtPointer)
{
   ...
   switch (item_no){
	...
	//cloudCB ();
        terrain = new GR_Terrain(1420, 31.5, -97.5, "n31.dte");
...
char             filename[120];
   ...
   switch (item_no) {
      ...
      case 1:
        if (!rec_entered) {
           sprintf(filename, "MOV%5.5d_%5.5d.bmp", process_id, frame_no);
