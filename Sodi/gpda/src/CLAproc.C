...
//
   ...
   nCols = ExecuteSQL( CLAhDbc, szSQL, cDelimiter, bColumnNames, bHTMLTable );
   if ( nCols > 0 ) {
...
//
   ...
   if (strcmp(iteltypes[j].filebase, "*") == 0) {
     ...
     strcat(clatemp, iteltypes[j].filebase);
     strcat(clatemp, ".rq");
   }
...
int             i, j, k, m;
   ...
   for (i=0; i<claFieldCount-4; i++) {
     fl_set_object_label(fd_itelfields->itel_fldselect[i], claFieldNames[i]);
     if ((strlen(claFieldNames[i]) > 1) && (strcmp(claFieldNames[i], "N/A") != 0))
       fl_set_button(fd_itelfields->itel_fldselect[i], PUSHED);
   }
   fl_set_object_label(fd_itelfields->itel_fldselect[48], claFieldNames[48]);
   fl_set_object_label(fd_itelfields->itel_fldselect[49], claFieldNames[49]);
   fl_set_object_label(fd_itelfields->itel_fldselect[50], claFieldNames[50]);
   fl_set_object_label(fd_itelfields->itel_fldselect[51], claFieldNames[51]);
...
SQLINTEGER      ret; 
   ...
   switch (item) {
	 ...
	 if (strstr(clatemp, "DATA_CASE") != NULL) {
	   ...
	   sscanf(fl_get_browser_line(fd_itelfields->clu_classes, k+1),
		  "%d %s", &i, chtemp);
	   fl_set_input(fd_itelfields->clu_closest, chtemp);
	   sprintf(clatemp, "%5.1f%%", P*100.0);
	   fl_set_input(fd_itelfields->clu_percent, clatemp);
	 }
       }
       fclose(clufp);
       break;
...
int             irc;
   ...
   if (strlen(fl_get_input(fd_itelfields->clu_closest)) > 1) {
     strcpy(clatemp, "Classifier determined hypothesis\n\n");
     strcat(clatemp, fl_get_input(fd_itelfields->clu_closest));
     strcat(clatemp, "\n\n to be closest match.\n\nAccept?");
