...
#include "rulebsc.h"
#include "ruledef.h"
#include "modulutl.h"   /* added 03-14-96 */

#include "agenda.h"

#if CERTAINTY_FACTORS   /* added 03-14-96 */
#include "cfdef.h"
#endif
...
#endif
#if CERTAINTY_FACTORS   /* added 03-14-96 */
   static BOOLEAN              CFEvaluation = WHEN_DEFINED;
#endif

/************************************************************/
/* InitializeAgenda: Initializes the activations watch item */
/*   and the CLIPS commands for manipulating the agenda.    */
/************************************************************/
...
#endif
#if CERTAINTY_FACTORS   /* added 03-14-96 */
   DefineFunction2("get-CF-evaluation",'w',
                   PTIF GetCFEvaluationCommand,"GetCFEvaluationCommand", "00");
   DefineFunction2("set-CF-evaluation",'w',
                   PTIF SetCFEvaluationCommand,"SetCFEvaluationCommand", "11w");
#endif
#endif
  }
...
globle BOOLEAN DetachActivation(vTheActivation)
  VOID *vTheActivation;
  {
   ...
   if (theActivation == NULL) CLIPSSystemError("AGENDA",1);
...
/*   occur during the evaluation of a rule's salience.      */
/************************************************************/
...
/* is refreshed. Otherwise, the agenda of the current module is  */ /* refreshed.                                                    */   /*===============================================================*/
...
/************************************************************/
globle SYMBOL_HN *SetCFEvaluationCommand()
  {
   DATA_OBJECT argPtr;
   char *argument, *oldValue;

   oldValue = CFEvaluationName(GetCFEvaluation());

   if (ArgCountCheck("set-CF-evaluation",EXACTLY,1) == -1)
     { return((SYMBOL_HN *) AddSymbol(oldValue)); }

   if (ArgTypeCheck("set-CF-evaluation",1,SYMBOL,&argPtr) == CLIPS_FALSE)
     { return((SYMBOL_HN *) AddSymbol(oldValue)); }

   argument = DOToString(argPtr);

   if (strcmp(argument,"when-defined") == 0)
     { SetCFEvaluation(WHEN_DEFINED); }
   else if (strcmp(argument,"when-activated") == 0)
     { SetCFEvaluation(WHEN_ACTIVATED); }
   else
     {
      ExpectedTypeError1("set-CF-evaluation",1,
...
/************************************************************/
globle SYMBOL_HN *GetCFEvaluationCommand()
  {
   ArgCountCheck("get-CF-evaluation",EXACTLY,0);

   return((SYMBOL_HN *) AddSymbol(CFEvaluationName(GetCFEvaluation())));
  }
...
/********************************************************************************/
globle BOOLEAN GetCFEvaluation()
  { return(CFEvaluation); }
...
static double EvaluateCF(vPtr)
 VOID *vPtr;
 {
  ...
  if ((rPtr->dynamicCF) != NULL && (GetCFEvaluation() != WHEN_DEFINED))
    {
      SetEvaluationError(CLIPS_FALSE);
      if (EvaluateExpression(rPtr->dynamicCF,&CFValue))
        {
          SetEvaluationError(CLIPS_TRUE);
          cfInformationError(ValueToString(rPtr->header.name));
        }
      else if (CFValue.type != INTEGER && CFValue.type != FLOAT)
        {
          cfNonNumberError();
          SetEvaluationError(CLIPS_TRUE);
          cfInformationError(ValueToString(rPtr->header.name));
        ...
        {
          ruleCF = (CFValue.type == FLOAT) ? ValueToDouble(CFValue.value) :
	                                    (double) ValueToLong(CFValue.value);
