...
GetDlgStuff(HWND hdlg, ConnInfo *ci)
{
	...
	GetDlgItemText(hdlg, IDC_UDS, ci->uds, sizeof(ci->uds));
}
...
char encoded_conn_settings[LARGE_REGISTRY_LEN];
	...
	sprintf(&connect_string[strlen(connect_string)],
		";READONLY=%s;PROTOCOL=%s;FAKEOIDINDEX=%s;SHOWOIDCOLUMN=%s;ROWVERSIONING=%s;SHOWSYSTEMTABLES=%s;CONNSETTINGS=%s;UDS=%s",
		ci->readonly,
		ci->protocol,
		ci->fake_oid_index,
		ci->show_oid_column,
...
copyAttributes(ConnInfo *ci, char *attribute, char *value)
{
	...
	mylog("copyAttributes: DSN='%s',server='%s',dbase='%s',user='%s',passwd='%s',port='%s',readonly='%s',protocol='%s', conn_settings='%s', uds='%s')\n", ci->dsn, ci->server,ci->database,ci->username,ci->password,ci->port,ci->readonly,ci->protocol,ci->conn_settings,ci->uds);
