...
public class COAOptimizer
{
                       ...
                       throws Exception
    {
        ...
        // COAOptimizerFit. We construct it with
        // the target amount of change passed in to this method.
        // ---------------------------------------------------------
        FitnessFunction myFunc =
            new COAOptimizerFit( a_targetChangeAmount );
        ...
        System.out.println( "\t" +
            COAOptimizerFit.getNumberOfCoinsAtGene(
        ...
        System.out.println( "For a total of " +
            COAOptimizerFit.amountOfChange(
                bestSolutionSoFar ) + " cents in " +
            COAOptimizerFit.getTotalNumberOfCoins(
    ...
    public void optimize( )
    {
      ...
      try {
        FileInputStream iInStream = new FileInputStream("ActiveOptions.dat");
        BufferedReader br = new BufferedReader(
                            new InputStreamReader(iInStream));
        StreamTokenizer st = new StreamTokenizer(br);
        st.commentChar(Integer.valueOf("#").intValue());
        st.nextToken();
        if (st.ttype == st.TT_NUMBER) {
          nOpt = (int)(st.nval);
        } else {
          System.out.println("No. of options expected");
        }
      ...
      //
      try {
        FileInputStream iInStream = new FileInputStream("ActiveTargets.dat");
        BufferedReader br = new BufferedReader(
                            new InputStreamReader(iInStream));
        String sTemp = br.readLine();
        do {
          if((sTemp.length() != 0) || (sTemp.charAt(0) != '#')) {
            nTgt = Integer.valueOf(sTemp).intValue();
            break;
          } else
            sTemp = br.readLine();
        } while(sTemp != null);
        ...
        for (int i=0; i<nTgt; i++) {
          br.readLine();
            ...
            try
            {
                int amount = Integer.parseInt( "71" );
