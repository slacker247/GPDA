...
**     3.1         Added MySQL/ODBC as method of accessing knowledge base      **
**     3.2         Enclosed LANL BP and SPEEDES stuff in IFDEFs                **
**     3.3         Implemented ALARMS as linked list & Message Handler         **
**     3.4         Implemented Single net with Multiple evidence sets          **
**                                                                             **
** *************************************************************************** */
/*                                                                             */
#define COMPILED __DATE__                  /* Today's date for version info    */
#define VERSION  3                         /* Application Version number       */
#define RELEASE  4                         /* Application Release number       */
#define SC_THREADS
/*                                                                             */
/* *************************************************************************** */
/*                                                                             */
...
#include "GL/gltk.h"
/*
**   Include the SPEEDES/Battle Manager interface stuff
*/
#ifdef LINKLANL
#include "parser.H"
#include "kepler.H"
...
char            *DATADIR;                         // Pointer to DATA directory
char            *BITMAPDIR;                       // Pointer to BITMAPS directory
char            *CLASS = "UNCLASSIFIED";          // Pointer to classification of run
...
char            chplan[40];                       // The current Battle Plan
char            chmsncon[40];                     // The current Mission Constraints
...
int             alarm_count = 0;
ALARM           *first_alarm = NULL;              // Pointer to first alarm in queue
ALARM           *last_alarm = NULL;               // Pointer to last alarm in queue
ALARM           *curr_alarm = NULL;               // Pointer to current alarm being processed
...
int             FinishUp();
void            PutOperator(int Severity, char *text);
...
extern void     CLAinit();
extern void     CLAshow(int xpos, int ypos, int width, int height, Window winid, int mode, char *fn);
extern void     CLAnext();
...
extern "C" void SC_update_con(char *newcon);
extern "C" void SC_update_obj(char *newobj);
...
void ErrorHandler(int signum, void *data)
{
...
//
   ...
   if (strcmp(parameters->get_string("input_source"), "Socket") == 0) {
      ...
      Net_init(grafport, grafhost);               // Initialize the socket connections
   }
...
//
   ...
   fl_add_signal_callback(SIGABRT,  ErrorHandler, &ErrorData);
   fl_add_signal_callback(SIGFPE,   ErrorHandler, &ErrorData);
   fl_add_signal_callback(SIGSEGV,  ErrorHandler, &ErrorData);
   fl_add_signal_callback(SIGWINCH, ErrorHandler, &ErrorData);
   fl_add_signal_callback(SIGSYS,   ErrorHandler, &ErrorData);
   fl_add_signal_callback(SIGQUIT,  ErrorHandler, &ErrorData);
   ...
   //
   sprintf(scline, "Version: %d.%d", VERSION, RELEASE);
   fl_set_object_label(fd_about->about_version, scline);
...
//
   ...
   fl_free_pixmap_pixmap(fd_bmc3->bmc_feed);
   sprintf (scline, "%s/%s", BITMAPDIR, "led-red.xpm");
   fl_set_pixmap_file(fd_bmc3->bmc_feed, scline);
   sprintf(scline, "%d", commstats[IF_SOCK].in_msgs);
   fl_set_input(fd_bmc3->bmc_incount, scline);
...
//
   sprintf(scline, "%d", alarm_count);
   fl_set_input(fd_bmc3->bmc_alarmcount, scline);
   first_alarm = (ALARM *)new(ALARM);
   first_alarm->prevalarm = NULL;
   first_alarm->nextalarm = NULL;
...
//
   if (SIMTEST != NULL) {
     SC_initialize();
     fl_set_button(fd_statsubs->subs_mon, YES);
   }
//
//   If External interface wanted, initialize it
//
#ifdef LINKCIC
   CIC_init(cicport, cichost);
   fl_set_button(fd_statsubs->subs_ext, YES);

   //const   pthread_mutexattr_t *mutexattr = PTHREAD_MUTEX_ERRORCHECK_NP;
   //pthread_mutex_init(&write_lock, mutexattr);
#endif
//
//   If DIS interface wanted, initialize it
//
   if (getenv("GPDADIS") != NULL) {
     display = getenv("REMOTE");
     strcpy(scline, "DIS.sh");
     //
     switch (DISpid = fork()) {
     case 0:
       if (display)
	 execlp(scline, scline, "-display", display, NULL);
       else
	 execlp(scline, scline, NULL);
       perror(scline);
       exit (255);
       break;
     case -1:
       fprintf(stderr, "Child process [%s] startup failed. PID = %d\n", scline, DISpid);
       break;
     default:
       fprintf(stderr, "DIS I/F process startup with PID = %d\n", DISpid);
       fl_set_button(fd_statsubs->subs_dis, YES);
       break;
     }
   }
//
...
//
   ...
   fl_set_button(fd_statsubs->subs_bm, YES);
#endif
//
//   Find out which sub-systems are available and tell user
//
#ifdef SC_THREADS
   fl_set_button(fd_statsubs->subs_multi, YES);
#endif
   if (stat("jtamv", &filestat) == 0)
     fl_set_button(fd_statsubs->subs_cop, YES);
   if (stat("odisim", &filestat) == 0)
     fl_set_button(fd_statsubs->subs_sim, YES);
   if ( OpenDatabase( &hEnv, &hDbc, szDSN, szUID, szPWD )) {
     fl_set_button(fd_statsubs->subs_kb, YES);
...
/*                                                                       */
/* --------------------------------------------------------------------- */
/*         M E S S A G E   H A N D L I N G    R O U T I N E S            */
/* --------------------------------------------------------------------- */
/*                                                                       */
#ifdef LINKCIC
#ifdef SC_THREADS
...
//
   ...
   case M_MINE: // MINE
     alarm_code  = ALARM_NONE;
     alarm_prior = PRIORITY_MED;
     strcpy(alarm_text, "MIDB Update");
     strcpy(filename, "\0");
     break;

   case M_STATS: // Box Scores
     alarm_code  = ALARM_NONE;
     alarm_prior = PRIORITY_LOW;
     strcpy(alarm_text, "Status Update");
     strcpy(filename, "\0");
     //
     ...
     //
     if (defcon > 0)                   SC_update_def(defcon);
     if (strcmp(chstat[0], "-1") != 0) SC_update_rp(atoi(chstat[0]));
     if (strcmp(chstat[1], "-1") != 0) SC_update_dea(chstat[1]);
     if (strcmp(chstat[2], "-1") != 0) SC_update_roe(chstat[2]);
     if (strcmp(chstat[3], "-1") != 0) SC_update_con(chstat[3]);
     if (strcmp(chstat[4], "-1") != 0) SC_update_obj(chstat[4]);
     if (strcmp(chstat[5], "-1") != 0) SC_update_plan(chstat[5]);
     break;
   ...
   case M_LIMIT: // Limit
     ...
     strcpy(filename, chevid);
     break;

   case M_INTEL: // Intel data
     alarm_code  = ALARM_LAUNCH;
     alarm_prior = PRIORITY_MED;
     strcpy(alarm_text, "Intel Data");
     strcpy(filename, chmission);
     //
     if (DSBevid(chmission, chevid) == -9) alarm_code = ALARM_NONE;
     break;
...
/*              (Used to talk with SPEEDES and Sim Commander)            */
/* --------------------------------------------------------------------- */
/*                                                                       */
...
int             flag = 1;
   ...
   */
   fl_free_pixmap_pixmap(fd_bmc3->bmc_feed);
   sprintf (scline, "%s/%s", BITMAPDIR, "led-green.xpm");
   fl_set_pixmap_file(fd_bmc3->bmc_feed, scline);
...
/* --------------------------------------------------------------------- */
/*                                                                       */
void PutOperator(int Severity, char *msg)
{
...
{
   if ((INFOfp = fopen("GPDAinfo.log", "w+")) != NULL) {
     fprintf(INFOfp, "%s", str);
     fprintf(INFOfp, "\n");
     fflush(INFOfp);
     fclose(INFOfp);
   }
...
/*                                                                       */
/* --------------------------------------------------------------------- */
/*            A L A R M   R O U T I N E S   S T A R T   H E R E          */
/* --------------------------------------------------------------------- */
/*                                                                       */
void NextAlarm(ALARM *alarmp)
{
int             item = 0;
FL_COLOR        color;
 
   switch (alarmp->priority) {
      case PRIORITY_CRIT:
	 color = FL_RED;
         alarm_active = TRUE;
         break;
   ...
   fl_set_object_label(fd_bmc3->alarm_text, alarmp->label);
...
ALARM           *alarmp;
   ...
   alarmp = (ALARM *)malloc(sizeof(ALARM));
   alarmp->alarm = alarm;
   alarmp->priority = priority;
   strcpy(alarmp->label, label);
   strcpy(alarmp->arg, arg);
   strftime(alarmp->tod, 24, "%D-%T", loctime);
   fl_gettime(&alarmp->timenow, &Microsec);

   last_alarm->nextalarm = alarmp;
   alarmp->prevalarm = last_alarm;
   alarmp->nextalarm = NULL;
   last_alarm = alarmp;

   NextAlarm(last_alarm);
...
INMSG           *p;
   ...
   ialarm = curr_alarm->alarm;
   strcpy(scline, curr_alarm->arg);
   ...
   if (item == 0) {
      switch (ialarm) {
        case ALARM_INTEL:
          CLAshow(winposX, winposY, winsizW, winsizH, mainwinID, 2, scline);
          break;

        case ALARM_LIMIT:
          sprintf(bmctemp, "Limit Exceeded:\n\n%s", scline);
   ...
   last_alarm = (ALARM *)curr_alarm->prevalarm;
   last_alarm->nextalarm = NULL;
   free(curr_alarm);
   curr_alarm = NULL;
...
//
   ...
   //
   sprintf(scline, "General Purpose Decision Aids - %d.%d", VERSION, RELEASE);
   fl_set_object_label(fd_bmc3->bmc_title, scline);
   //
...
//
   ...
   fl_set_input(fd_sitaware->sit_dea, chdea);
   ...
   fl_set_input(fd_sitaware->sit_roe, chroe);
...
//
   ...
   for (i=0; i<6; i++) {
     ...
     fgets(scline, 128, infp);
     fscanf(infp, "%s %d", chlabel, &j);
     strsub(chlabel, '_', ' ');
     ...
     if (j == 0) {
       ...
       for (k=0; k<j; k++) {
	 fscanf(infp, "%s %d", chlabel, &n);
	 fl_add_chart_value(fd_sitaware->sit_durable[i], (double)n, chlabel, FL_RED+k);
       }
     }
   }
//
...
int             item = 0;
   ...
   if (DISpid > 0) kill(DISpid, SIGKILL);
...
char            filename[64];
   ...
   switch (item) {
     case 1:  // Concept Discovery (Subdue)
       CLUshow(winposX, winposY, winsizW, winsizH, mainwinID);
       break;

     case 2:  // Hypothesis classification
       CLAshow(winposX, winposY, winsizW, winsizH, mainwinID, 1, NULL);
       break;
...
int             item = item_no;
   ...
   switch (item) {
     ...
     case 3: // Print Controls
       fl_show_form(fd_printform->printform, FL_PLACE_CENTER,FL_FULLBORDER,
		    "Print Control");
       break;
     ...
     case 7: // About
       sprintf(scline, "Compiled: %s", COMPILED);
       fl_set_object_label(fd_about->about_date, scline);
       fl_load_browser(fd_about->about_copyright, "copyright.txt");
       fl_show_form(fd_about->About, FL_PLACE_CENTER,FL_FULLBORDER, "About GPDA");       
       break;
...
void prttimeoutCB(int tid, void *stuff)
{
int             maxline;

   maxline = fl_get_browser_maxline(fd_bmc3->act_windows);
   sprintf(scline, "xwd -name %s | convert - gif:/tmp/temp.gif", 
	   fl_get_browser_line(fd_bmc3->act_windows, maxline));
   system(scline);
   system("convert -rotate 90 -geometry 80% /tmp/temp.gif /tmp/temp2.gif");
   system("convert /tmp/temp2.gif ps:/tmp/temp2.ps");
   sprintf(scline, "convert -geometry +20+20 /tmp/temp2.ps /tmp/%s.ps",
	   fl_get_browser_line(fd_bmc3->act_windows, maxline));
   system(scline);
   sprintf(scline, "lp /tmp/%s.ps",
	   fl_get_browser_line(fd_bmc3->act_windows, maxline));
   system(scline);
...
void bmcprintCB(FL_OBJECT *object, long item_no)
{
int             item = item_no;

   switch (item) {
     case 0: // Print
       fl_hide_form(fd_printform->printform);
       fl_add_timeout((long)1L*500L, prttimeoutCB, NULL);
       break;
...
FILE            *fp;
int             item = item_no;
extern int      DSBevid(char *m, char *buf); 
   ...
   switch (item) {
     ...
     case 2:
       Alarm(ALARM_LAUNCH, "Potential Event", PRIORITY_HI, "TCT_24");
       break;
     ...
     case 4:
       sprintf(scline, "Tank  0.47  0.16  2003:07:10:0:0:0 30.0 29.9 121.6 0.0");
       DSBevid("DRE", scline);
       break;
...
void SC_update_con(char *newcon)
{
   if (!BMCinited) return;

   if (OperQuery("Situation Monitor", "Mission Constraints", " ", newcon) != 0) {
      fl_set_object_label(fd_bmc3->msncon_val, newcon);
      fl_redraw_form(fd_bmc3->bmc3);              // Clean up any overdraws
      strcpy(chmsncon, newcon);
      //update_SC = OP_ROE;
   }
}

void SC_update_obj(char *newobj)
{
   if (!BMCinited) return;

   if (OperQuery("Situation Monitor", "Mission Objectives", " ", newobj) != 0) {
      fl_set_object_label(fd_bmc3->msnobj_val, newobj);
      fl_redraw_form(fd_bmc3->bmc3);              // Clean up any overdraws
      strcpy(chmsnobj, newobj);
      update_SC = OP_MSN_OBJ;
   }
}
