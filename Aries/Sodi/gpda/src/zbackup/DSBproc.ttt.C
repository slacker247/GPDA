...
**   Things to do:
**
**     - Change 'Effectiveness' processing to handle other than 6 level nets only.
**     - Merge DS algorithm and DSB GUI initialization files.
**     - Implement 'Conclusion' level node update of 'Evidence' files.
**     - Allow NxN networks.
**     - Provide GUI-based editors for all startup files.
**     - Change evidence storage to time-ordered linked-list ???
**
*********************************************************************************/
#include <stdio.h>
#include <stdlib.h>
...
void
fl_set_object_helper(FL_OBJECT *ob, char *str)
{
}
...
// 
   ...
   for (j=0; j<num_types; j++) {
     ...
     fl_addto_choice(fd_assess->asstype_menu, dsbtypes[j].ChType);
   }
   fclose(typefp);
#ifndef NOGL
   H3Dinit();
...
void DSBshow(int xpos, int ypos, int width, int height, Window mainwinID)
{
   ...
   if(!fl_form_is_visible(fd_assess->assess) ) {
     DSBx = xpos;
     DSBy = ypos;
     fl_transient();
     fl_winposition(xpos, ypos);
     fl_initial_winsize(width, height);
     DSBwinid = fl_prepare_form_window(fd_assess->assess,
                                     FL_PLACE_POSITION,FL_TRANSIENT, "Assess-Operator");
     ...
     fl_set_form_atclose(fd_assess->assess, DSBclose, 0);
     fl_set_form_title(fd_assess->assess, "Assess-Operator");
     StoreActiveEntry("Assess-Operator");
   }

   fl_set_choice(fd_assess->asstype_menu, 1);
   input_menuCB(NULL, (long)1);
...
const char      *fname;
   ...
   if (EvidModified) {
     if (fl_show_question("Evidence changed! Save?", 0) == TRUE) {
       ...
       if (fname != NULL) DSsave(fname);               // Save the updated evidence
     }
   }
   EvidModified = FALSE;
...
FL_OBJECT       *ipipm;
   ...
   if (EvidModified) {
     ...
     if (EvidModified == TRUE) {
       fname = fl_show_fselector("Evidence File", "./DSBFiles", "*.evid", NULL);
       if (fname != NULL) DSsave("DSBFiles/NewEvidence.evid");        // Save the updated evidence
     }
   }
   EvidModified = FALSE;
 
   curr_type = fl_get_choice(fd_assess->asstype_menu)-1;
...
//
   ...
   for (level=0; level<Nlevels; level++) {
     ...
     for (i=0; i<Nnodes[level]; i++) {
       ...
       if (level == Nlevels-1) {
	 strsub(chtemp, '\n', ' ');
	 fl_set_object_label(fd_assess->assm_text[i],  chtemp);
	 fl_activate_object(fd_assess->assess_graph[i]);
	 //
	 DS_AlgoGetAssmntThresholds(i, bthresh, tthresh);
   ...
   fl_set_button(fd_assess->assess_graph[curr_assess], 1);
//
//   Initialize assessment charts
//
   fl_clear_chart(fd_assess->belief_pie);
   fl_set_chart_bounds(fd_assess->belief_pie, 0.0, 1.0);
   fl_add_chart_value(fd_assess->belief_pie, (double)0.0, "1", ChartColor[1]);
   fl_add_chart_value(fd_assess->belief_pie, (double)0.0, "2", ChartColor[2]);
   fl_add_chart_value(fd_assess->belief_pie, (double)0.0, "3", ChartColor[3]);
   fl_add_chart_value(fd_assess->belief_pie, (double)0.0, "4", ChartColor[4]);
   fl_add_chart_value(fd_assess->belief_pie, (double)0.0, "5", ChartColor[5]);
   fl_add_chart_value(fd_assess->belief_pie, (double)0.0, "6", ChartColor[6]);
   fl_clear_chart(fd_assess->disbelief_pie);
   fl_set_chart_bounds(fd_assess->disbelief_pie, 0.0, 1.0);
   fl_add_chart_value(fd_assess->disbelief_pie, (double)0.0, "1", ChartColor[1]);
   fl_add_chart_value(fd_assess->disbelief_pie, (double)0.0, "2", ChartColor[2]);
   fl_add_chart_value(fd_assess->disbelief_pie, (double)0.0, "3", ChartColor[3]);
   fl_add_chart_value(fd_assess->disbelief_pie, (double)0.0, "4", ChartColor[4]);
   fl_add_chart_value(fd_assess->disbelief_pie, (double)0.0, "5", ChartColor[5]);
   fl_add_chart_value(fd_assess->disbelief_pie, (double)0.0, "6", ChartColor[6]);
   ...
   //
   if (fl_get_xyplot_numdata(fd_assess->conf_plot, 1) > 0) {
     fl_get_xyplot_overlay_data(fd_assess->conf_plot, 1, XtOld, YbOld, &osteps);
     fl_get_xyplot_overlay_data(fd_assess->conf_plot, 2, XtOld, YpOld, &osteps);
   }
   //
   //   Notify the user if new data and/or nodes are available to be processed
   //   after reseting all indicators to 'No'
   //
   sprintf (dsbtemp, "%s/%s", DSBitMaps, "led-red.xpm");
   ipipm = fd_assess->assess_newdata;
   fl_show_object(ipipm);
   fl_free_pixmap_pixmap(ipipm);
   fl_set_pixmap_file(ipipm, dsbtemp);
   ipipm = fd_assess->assess_newnode;
   fl_show_object(ipipm);
   fl_free_pixmap_pixmap(ipipm);
   fl_set_pixmap_file(ipipm, dsbtemp);
   fl_hide_object(fd_assess->assess_newmsn);
   fl_set_button(fd_network->net_newnode1, FALSE);
   //
   if ((fp = fopen("NewNode.lock", "r")) != NULL) {
     fscanf(fp, "%s", chtemp);
     strsub(chtemp, '_', ' ');
     if (strcmp(fl_get_choice_text(fd_assess->asstype_menu), chtemp) == 0) {
       sprintf (dsbtemp, "%s/%s", DSBitMaps, "led-green.xpm");
       ipipm = fd_assess->assess_newnode;
       fl_show_object(ipipm);
       fl_free_pixmap_pixmap(ipipm);
       fl_set_pixmap_file(ipipm, dsbtemp);
       //printf("%s -%s-\n", fl_get_choice_text(fd_assess->asstype_menu)-1, chtemp);
       fl_set_button(fd_network->net_newnode1, TRUE);
     }
     fclose(fp);
   }
   //
   //   Notify the user if a plan is available to be processed
   //
   k = fl_get_choice(fd_assess->asstype_menu)-1;
...
// 
   fl_set_object_label(fd_assess->case_id, chcaseid);
   fl_set_object_label(fd_network->case_id, chcaseid);
...
//
     if ((etype == 2)) {
       ...
       //
       if (!fl_get_button(fd_assess->assess_secure)) {
         error = savedNet_readFile(fname);
         if (!error) {
	   ...
	   nodes = subnode;
         ...
         if (!error) {
           ...
           sscanf(dsbtemp, "%d  %s\n", &nlevels, chmission);
//ttt -- changed:  if (sublevel > nlevels) sublevel = nlevels; ------------ttt
...
char          dsbtemp[128];
   ...
   while (!feof(infile)) {
     ...
     while (1) {
       ...
       if (strstr(temp2, "))")) last_field = TRUE;

       strsub(temp2, ')', ' '); strsub(temp2, quote, ' ');
       //fprintf(stderr, "%s %s\n", temp1, temp2);

            if (strcmp(temp1, "(Time") == 0) sscanf(temp2, "%f)", &time);
       else if (strcmp(temp1, "(Level") == 0) sscanf(temp2, "%d)", &level);
       else if (strcmp(temp1, "(Duration") == 0) sscanf(temp2, "%f)", &duration);
       else if (strcmp(temp1, "(Type") == 0) sscanf(temp2, "%d)", &type);
       else if (strcmp(temp1, "(External-Level") == 0) sscanf(temp2, "%d)", &ext_level);
       else if (strcmp(temp1, "(Dur-Unit") == 0) sscanf(temp2, "%s", chdur);
       else if (strcmp(temp1, "(Mission-Domain") == 0) sscanf(temp2, "%s", chmission);
       else if (strcmp(temp1, "(External-Filename") == 0) sscanf(temp2, "%s", filename);
       else if (strcmp(temp1, "(Disbelief") == 0) sscanf(temp2, "%f)", &disbelief);
       else if (strcmp(temp1, "(Arg-3") == 0) sscanf(temp2, "%s", charg3);
       else if (strcmp(temp1, "(Time-Unit") == 0) sscanf(temp2, "%s", chtunit);
       else if (strcmp(temp1, "(Arg-1") == 0) sscanf(temp2, "%s", charg1);
       else if (strcmp(temp1, "(Node") == 0) sscanf(temp2, "%d)", &node);
       else if (strcmp(temp1, "(Description") == 0) sscanf(temp2, "%s", chdesc);
       else if (strcmp(temp1, "(Source") == 0) sscanf(temp2, "%s", chsource);
       else if (strcmp(temp1, "(External-Node") == 0) sscanf(temp2, "%d)", &ext_node);
       else if (strcmp(temp1, "(Arg-2") == 0) sscanf(temp2, "%s", charg2);
       else if (strcmp(temp1, "(Belief") == 0) sscanf(temp2, "%f)", &belief);
       
       if (last_field) break;
     }
     DS_AlgoGetNodeName(node-1, level-1, dsbtemp);
     strsub(dsbtemp, ' ', '_');

     fprintf(outfile, "%3d  %-30s  %6.2f  %6.2f  %8.2f %s  %8.2f  %s  %-8s  %-8s  %-8s  %-16s %4d %4d   %s\n",
	    type, dsbtemp, belief, disbelief, time, chtunit,
...
Window          winid;
   ...
   switch (item_no)
   {
	 ...
	 if ((fp = fopen(dsbtemp, "r")) == NULL) return;
       ...
       fl_set_input(fd_dsstory->story_mission, fl_get_choice_text(fd_assess->asstype_menu));
...
char            chfile[128];
   ...
   k = fl_get_choice(fd_assess->asstype_menu)-1;
   fl_set_object_label(fd_network->assess_type,
		       fl_get_choice_text(fd_assess->asstype_menu));

   if (strcmp(dsbtypes[k].filebase, "*") == 0) {
     strcpy(chfile, fl_show_fselector("Network file", "./DSBFiles", "*.dsbn", NULL));
     if (strlen(chfile) == 0) return;
   } else {
...
char            chfile[128];
   ...
   //
   if (strcmp(dsbtypes[k].filebase, "*") == 0) {
     strcpy(chfile, fl_show_fselector("Evidence file", "./DSBFiles", "*.evid", NULL));
     if (strlen(chfile) == 0) return;
   } else {
     strcpy(chfile, "DSBFiles/");
     strcat(chfile, dsbtypes[k].filebase);
...
//
   ...
   for (Hlev=0; Hlev<k-1; Hlev++) {
     ...
     for (i=0; i<DS_AlgoGetLevelWidth(Hlev); i++) {
       ...
       //for (j=0; j<Nnodes[Hlev+1]; j++) {
       for (j=0; j<DS_AlgoGetLevelWidth(Hlev+1); j++) {
         DS_AlgoGetMatrix(i, Hlev, j, Hlev, In_String, Out_String,
...
Window          thiswin; 
   ...
   if (DoBP) {
     DS_AlgoChangeImpacts_BP(LinkConstrain, 0.5, 6);    // Do the back propagation
...
//
   ...
   if ((fl_mouse_button() == FL_RIGHTMOUSE) &&
       (fl_get_button(fd_network->net_plan) == TRUE)) {
     /*
     strcpy(dsbtemp, nodeinfo[icol][irow].extern_name);
     if (strlen(dsbtemp) > 1) {
       strcat(dsbtemp, ".gif");
       fl_show_form(fd_netsnap->netsnap,FL_PLACE_CENTER,FL_FULLBORDER,"External Net");
       ...
       flimage_sdisplay(image, FL_ObjWin(fd_netsnap->net_image));
#endif
     }
     fl_update_display(0);
     */
     ...
     pupinfo.belief    = results[1];
     pupinfo.disbelief = results[2];
     pupinfo.time      = results[3];
     //
     strcpy(dsbtemp, fl_get_object_label(fd_network->node_label[icol][irow]));
     strsub(dsbtemp, '\n', ' ');
     ...
     //
     PUPshow(DSBx, DSBy, DSBw, DSBh, DSBwid, pupinfo);
   }
   ...
   if (fl_mouse_button() == FL_LEFTMOUSE) {
     ...
     menu = fl_defpup(fl_winget(),
		      "Edit %t|New Evidence|Node Override|Node Copy|Node Paste|Link Edit");
...
H3DINFO         h3dinfo;
   ...
   switch (item_no) {
     ...
     case 13: // Snapshot
       ...
       n = fl_get_choice(fd_assess->asstype_menu)-1;
...
char            chfile[128];
   ...
   switch (item_no) {
     case 0: // Get 'File' selection
       k = fl_get_menu(fd_network->netfile_menu)-1;
       switch (k) {
         case 0:  executeCB(NULL, 0); break;                //+ Run
         case 1:  executeCB(NULL, 1); break;                //+ Step
         case 2:  input_displayCB(NULL, 1); break;          //+ Edit
         case 3:  neteddoneCB(NULL, 14); break;             //+ Save
         case 4:  neteddoneCB(NULL, 13); break;             //+ Snapshot
         default: break;
       }
       break;
     ...
     case 2: // Get 'Operations' selection
       ...
       switch (k) {
         case 0:  neteddoneCB(NULL,  9); break;             // Intersection
         case 1:  neteddoneCB(NULL, 10); break;             // Union
...
FL_IMAGE        *Limage, *Rimage; 
   ...
   switch (item_no)
   {
	 ...
	 savedNet_levels[level].numNodes = DS_AlgoGetLevelWidth(level);
	 strcpy(savedNet_levels[level].name,
                fl_get_object_label(fd_network->level_label[level]));
	 strsub(savedNet_levels[level].name, '\n', '_');

         for (i=0; i<n; i++) {
           DS_AlgoGetResult(i, level, results);
           savedNet_nodes[level][node].belief = results[1];
           savedNet_nodes[level][node].disbelief = results[2];
           savedNet_nodes[level][node].time = results[4];
	   strcpy(savedNet_nodes[level][i].name,
                  fl_get_object_label(fd_network->node_label[i][level]));
	   strsub(savedNet_nodes[level][i].name, '\n', '_');
         }
       }
	 ...
	 if (savedNet_numLevels == 6) {
	   fl_mapcolor(FL_FREE_COL1+1, 255, 172, 122);
	   ...
	   for (j=0; j<savedNet_numLevels; j++) {
	     ...
	     for (i=0; i<n; i++) {
	       ...
	       if (savedNet_nodes[j][i].belief != 0.0)
		 ...
		 Rbelief = -99999.0;
	       ...
	       if (Rbelief < 1.0) boxcolor = FL_FREE_COL1+1;
     ...
     case 9: // Evidence import
       item  = fl_get_choice(fd_assess->asstype_menu) - 1;
       //
       //   Get the mission network file name
       //
	 ...
	 strcpy(chfile, "DSBFiles/");
	 strcat(chfile, dsbtypes[item].filebase);
	 strcat(chfile, ".dsbn");
       }
       //fname = dsbtypes[item-1].afilename;
       //
       item  = fl_get_choice(fd_assess->assess_import);
       tempfn = fl_show_fselector("Evidence file", "./", "*.pins", NULL);
       if (tempfn == NULL) break;
       strcpy(infname, tempfn);       
       switch (item) {
         case 1: // PINS Import
           sprintf(otfname, "TempEvid.%d", getpid() );
	 ...
	 fscanf(fp, "%d %s %d %f %f %c\n", &nlevels, chcase, &mode, &Ts, &Te, &Tu);
         fl_mapcolor(FL_FREE_COL1+1, 155, 100, 100);
         for (j=0; j<nlevels; j++) {
	 ...
	 w = 0.50 * Rimage->w; h = 0.50 * Rimage->h;
	 ...
	 w = 0.50 * Limage->w; h = 0.50 * Limage->h;
...
char            chfile[128];
       ...
       item  = fl_get_choice(fd_assess->asstype_menu);

       if (strcmp(dsbtypes[item-1].filebase, "*") == 0) {
	 strcpy(chfile, fl_show_fselector("Evidence file", "./DSBFiles", "*.evid", NULL));
	 if (strlen(chfile) == 0) return;
       } else {
	 strcpy(chfile, "DSBFiles/");
...
float           results[6];
     ...
     //   Execute the network
     //
     DSinit(chtemp);                                        // Load the network
...
Window          winid;
   ...
   if (strcmp(dsbtypes[k].filebase, "*") == 0) {
     strcpy(filename, fl_show_fselector("Explanation file", "./DSBFiles", "*.help", NULL));
...
int             level, i, n, item = 0;
   ...
   for (i=0; i<6; i++) {
     ...
     fl_deactivate_object(fd_assess->assess_graph[i]);
   }
 
   level = fl_get_choice(fd_assess->assess_level) - 1;
   for (i=0; i<DS_AlgoGetLevelWidth(level); i++) {
     strcpy(dsbtemp, fl_get_object_label(fd_network->node_label[i][level]));
     strsub(dsbtemp, '\n', ' ');
     fl_set_object_label(fd_assess->assm_text[i],  dsbtemp);
     fl_activate_object(fd_assess->assess_graph[i]);
   }

   fl_set_button(fd_assess->assess_graph[0], 1);
   assess_selectCB(NULL, (long)0);
}
...
//
     ...
     /*
     //
     //   We carry time in minutes. 
     //
     if (toupper(timeUnit) == 'S') Tx = Tx*60.0;
     else if (toupper(timeUnit) == 'H') Tx = Tx/60.0;
     else if (toupper(timeUnit) == 'D') Tx = Tx/60.0/24.0;
     */
     graph = fd_assess->conf_plot;
     //
     ...
     fl_set_xyplot_overlay_type(graph, 3, FL_DOTTED_XYPLOT);     // Threshold curves
     ...
     Xover2[0] = 0.0;
     Yover2[0] = 0.0;
     fl_set_xyplot_data(graph, Xover2, Yover2, 1, "Confidence of Assessment",
                          "Time (Min)","%");
     //
     Xover2[0] = 0.0;
     Yover2[0] = currentype.assessment[curr_assess].Bthreshold*100.0;
     Xover2[1] = currentype.assessment[curr_assess].Tthreshold*Tconvert[time_unit];
     Yover2[1] = currentype.assessment[curr_assess].Bthreshold*100.0;
     Xover2[2] = currentype.assessment[curr_assess].Tthreshold*Tconvert[time_unit];
     Yover2[2] = 0.0;
     fl_add_xyplot_overlay(graph, 3, Xover2, Yover2, 3, FL_BLACK);
     //
     for (i=0; i<6; i++) {
       fl_set_object_label(fd_assess->ds_match[i], " ");
       fl_set_object_label(fd_assess->assess_belief[i+1], " ");
       fl_set_object_label(fd_assess->assess_disbelief[i+1], " ");
       fl_replace_chart_value(fd_assess->belief_pie, i+1, (double)0.0,
				ChartLabel[i], ChartColor[i]);
       fl_replace_chart_value(fd_assess->disbelief_pie, i+1, (double)0.0,
				ChartLabel[i], ChartColor[i]);
     }
     //
     n = -1;
...
//
	 ...
	 if (dofusion == TRUE) {
       ...
       /*
       //
       //   We carry time in minutes. Convert to whatever the user has specified
       //
       if (toupper(timeUnit) == 'S') Xt[n] = Xt[n]*60.0;
       else if (toupper(timeUnit) == 'H') Xt[n] = Xt[n]/60.0;
       else if (toupper(timeUnit) == 'D') Xt[n] = Xt[n]/60.0/24.0;
       */
       fl_add_xyplot_overlay(graph, 1, Xt, Yb, n+1, FL_BLUE);
       fl_add_xyplot_overlay(graph, 2, Xt, Yp, n+1, FL_RED);
       //
       //   Update selection table and charts
       //
       NodeNo = 0;
       Bmax = -1.0;
       Dmin =  2.0;
       ...
       for (i=0; i<DS_AlgoGetLevelWidth(level-1); i++) {
         ...
         fl_set_object_label(fd_assess->assess_belief[i+1], chline);
         fl_replace_chart_value(fd_assess->belief_pie, i+1, (double)results[1],
				ChartLabel[i], ChartColor[i]);
         sprintf(chline, "%d%%", (int)(results[2]*100.0+0.5));
         fl_set_object_label(fd_assess->assess_disbelief[i+1], chline);
         fl_replace_chart_value(fd_assess->disbelief_pie, i+1, (double)results[2],
				ChartLabel[i], ChartColor[i]);
