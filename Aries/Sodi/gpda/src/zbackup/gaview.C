...
// make the application widget, grab resource, and parse command line
  ...
  toplevel = XtAppInitialize(&theAppData.appc, APP_CLASS, 
...
// report status
  ...
  case 1:
    ...
    strcpy(Text_Which_Genome, "Bin-to-Dec");
    break;
  default:
    cout << "real number";
    strcpy(Text_Which_Genome, "Real");
    break;
  }
  cout << "\n";
  cout << "  algorithm: ";
  ...
  case 0:
    ...
    strcpy(Text_Which_GA, "Incremental");
    break;
  case 1:
    cout << "simple (non-overlapping population)";
    strcpy(Text_Which_GA, "Simple");
    break;
  case 3:
    cout << "deme (multiple populations)";
    strcpy(Text_Which_GA, "Deme");
    break;
  default:
    cout << "steady-state (overlapping population)";
    strcpy(Text_Which_GA, "Steady-State");
    break;
  }
  cout << "\n" << endl;
...
static char     txt[62];
  ...
  sprintf(txt, "Generation # %d,   Best Score = %f", ga->generation(),
  ...
  sprintf(bigstring, "\n");
  sprintf(littlestr, "Statistics since initialization:\n");
  strcat (bigstring, littlestr);
  sprintf(littlestr, " Current generation ..... %10d\n", ga->statistics().generation());
  strcat (bigstring, littlestr);
  sprintf(littlestr, " Current convergence .... %10.6f\n", ga->statistics().convergence());
  strcat (bigstring, littlestr);
  sprintf(littlestr, " Total # selections ..... %10ld\n", ga->statistics().selections());
  strcat (bigstring, littlestr);
  sprintf(littlestr, " Total # crossovers ..... %10ld\n", ga->statistics().crossovers());
  strcat (bigstring, littlestr);
  sprintf(littlestr, " Total # mutations ...... %10ld\n", ga->statistics().mutations());
  strcat (bigstring, littlestr);
  sprintf(littlestr, " Total # replacements ... %10ld\n", ga->statistics().replacements());
  strcat (bigstring, littlestr);
  sprintf(littlestr, " Total # genome evals ... %10d\n", 250);
  strcat (bigstring, littlestr);
  sprintf(littlestr, " Total # pop evals ...... %10d\n", 1);
  strcat (bigstring, littlestr);
  sprintf(littlestr, " Maximum score .......... %10.6f\n", 0.987398);
  strcat (bigstring, littlestr);
  sprintf(littlestr, " Minimum score .......... %10.6f\n\n", 0.017648);
  strcat (bigstring, littlestr);
  sprintf(littlestr, "Current statistics:\n");
  strcat (bigstring, littlestr);
  sprintf(littlestr, " Avg of all scores ...... %10.6f\n", ga->statistics().online());
  strcat (bigstring, littlestr);
  sprintf(littlestr, " Avg of max scores ...... %10.6f\n", 0.987398);
  strcat (bigstring, littlestr);
  sprintf(littlestr, " Avg of min scores ...... %10.6f\n", 0.017648);
  strcat (bigstring, littlestr);
  sprintf(littlestr, " Mean score initial pop . %10.6f\n",
	  ga->statistics().initial(GAStatistics::Mean));
  strcat (bigstring, littlestr);
  sprintf(littlestr, " Max score initial pop .. %10.6f\n",
	  ga->statistics().initial(GAStatistics::Maximum));
  strcat (bigstring, littlestr);
  sprintf(littlestr, " Min score initial pop .. %10.6f\n",
	  ga->statistics().initial(GAStatistics::Minimum));
  strcat (bigstring, littlestr);
  sprintf(littlestr, " Std. dev. initial pop .. %10.6f\n",
	  ga->statistics().initial(GAStatistics::Deviation));
  strcat (bigstring, littlestr);
  sprintf(littlestr, " Diversity initial pop .. %10.6f\n",
	  ga->statistics().initial(GAStatistics::Diversity));
  strcat (bigstring, littlestr);
  sprintf(littlestr, " Mean score current pop . %10.6f\n",
	  ga->statistics().current(GAStatistics::Mean));
  strcat (bigstring, littlestr);
  sprintf(littlestr, " Max score current pop .. %10.6f\n",
	  ga->statistics().current(GAStatistics::Maximum));
  strcat (bigstring, littlestr);
  sprintf(littlestr, " Min score current pop .. %10.6f\n",
	  ga->statistics().current(GAStatistics::Minimum));
  strcat (bigstring, littlestr);
  sprintf(littlestr, " Std. dev. current pop .. %10.6f\n",
	  ga->statistics().current(GAStatistics::Deviation));
  strcat (bigstring, littlestr);
  sprintf(littlestr, " Diversity current pop .. %10.6f\n",
	  ga->statistics().current(GAStatistics::Diversity));
  strcat (bigstring, littlestr);
  strcat (bigstring, "\0");
  xstr = XmStringCreateLtoR(bigstring, "charset1");
...
char            chtemp[128];
  ...
  textarea = XtVaCreateManagedWidget("", xmLabelWidgetClass, form,
                 XmNwidth,            300,
		 //XmNheight,           100,
                 XmNmarginHeight,     2,
  ...
  sprintf(chtemp, "Algorithm: %s", Text_Which_GA);
  XmString algortitle = XmStringCreateLtoR(chtemp, "charset1");
  Widget algorwidget  = XtVaCreateManagedWidget("class", xmLabelWidgetClass, form,
                 XmNheight,           25,
                 XmNalignment,        XmALIGNMENT_BEGINNING,
                 XmNlabelType,        XmSTRING,
                 XmNlabelString,      algortitle,
						//XmNfontList,         fontlist,
		 ...
		 XmNleftWidget,       textarea,
                 ...
                 NULL);
  XmStringFree(algortitle);

  sprintf(chtemp, "Genome: %s", Text_Which_Genome);
  XmString genometitle = XmStringCreateLtoR(chtemp, "charset1");
  Widget genomewidget  = XtVaCreateManagedWidget("class", xmLabelWidgetClass, form,
                 XmNheight,           25,
                 XmNalignment,        XmALIGNMENT_BEGINNING,
                 XmNlabelType,        XmSTRING,
                 XmNlabelString,      genometitle,
						//XmNfontList,         fontlist,
						//XmNforeground,       fg_red,
                 XmNshadowThickness,  2,
		 XmNbottomAttachment, XmATTACH_WIDGET,
		 XmNbottomWidget,     algorwidget,
                 XmNleftAttachment,   XmATTACH_WIDGET,
		 XmNleftWidget,       textarea,
                 XmNrightAttachment,  XmATTACH_FORM,
                 NULL);
  XmStringFree(genometitle);
