...
GR_Tearth::GR_Tearth (char* filename, int component, int xsize, int ysize)
{
   ...
   quadTexObj = gluNewQuadric();
   gluQuadricDrawStyle(quadTexObj, (GLenum) GLU_FILL);
   gluQuadricOrientation(quadTexObj, (GLenum) GLU_OUTSIDE);
   gluQuadricTexture(quadTexObj, GL_TRUE);
   gluQuadricNormals(quadTexObj, (GLenum) GLU_SMOOTH);
...
GR_Tearth::draw_sphere (int n, Boolean nflag, Boolean tflag)
{
   ...
   if (tflag) GR_rot(-90.0, 'x');

   gluSphere(quadTexObj, 1.0, 128, 128);
/*
