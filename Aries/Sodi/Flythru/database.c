...
#define SIZE            1208
#define MAXLEVEL        8               /* Maximum fractal level */
#define NUMRANDS        191             /* Random number table size */
#define SKY             50.0
#define NUM_DROPS       80              /* Number of rain drops for erosion */

int             clouds;
int             Rebuild = 1;            /* Rebuild display list in next display? */
int             Level   = 4;            /* Levels of recursion for fractals */
int             DrawAxes = 0;           /* Should we draw some axes? */

float           *A;                     /* Terrain array */
float           AMax = 1.0;             /* Maximum elevation encountered */
...
float           ResultVert[3];          /* Resulting vertex from recursive FMR */
...
static float paper_plane_vertexes[] = { /* Define paper airplane model */
/*Nx  Ny  Nz   Vx     Vy    Vz */
...
static void create_terrain_sine(void);
static void create_terrain_frac(void);
static void create_terrain_dted(void);
static void create_terrain_usgs(void);
static void erode_terrain(void);
static void color_terrain(void);
       float get_elevation(int i, int j);
...
/************************ Terrain Stuff ************************/
/***************************************************************/
void init_terrain(int terrain_type)
{
    ...
    //if (A != NULL) free(A);
    if (terrain_type == TERR_DTED) create_terrain_dted();
    else if (terrain_type == TERR_FRAC) create_terrain_frac();
    else if (terrain_type == TERR_USGS) create_terrain_usgs();
    else create_terrain_sine();
    //Init_y = GridDim/2.0;               /* Initial Y in middle of South edge */
    Init_x = GridDim/2.0;

    //erode_terrain();
    color_terrain();
    init_cells();
    fprintf(stderr, "All initialization complete...\n");
}

float get_elevation(int i, int j)
{

  //printf("Altitude = %f, scaled = %f\n", A[i*GridDim + j], A[i*GridDim + j] * Zscale); 
   return(A[i*GridDim + j] * Zscale);
}

/*
** Create a sinusoidal-based terrain
*/
static void create_terrain_sine(void)
{
...
int             hillsize;
     ...
     *  where we make a maximum height 'hill' that will later be
...
*/
void FractalMountain(GLfloat v1[3], GLfloat v2[3], GLfloat v3[3], int pegged[3])
{ 
...
int             pegged[3] = { 1, 1, 1 };
    ...
    InitRandTable(time(NULL));          /* New random numbers to make a mountain */

    hillsize = GRID_RANGE / 12;

    A = (float*)calloc(GridDim * GridDim, sizeof(float));
    /*
    **  Initialize elevation to zero
    **/
    for (r = 0; r < GridDim; r++)
        for (c = 0; c < GridDim; c++) A[r * GridDim + c] = 0.0;
    /*
    ** Build the terrain
    */
    FractalMountain(v1, v2, v3, pegged);
    /* 
    ** Clamp the elevation of the terrain to 0.0 - 1.0
    */
    for (r = 1; r < GridDim; r++)
	for (c = 1; c < GridDim; c++) {
	    A[r * GridDim + c] = A[r * GridDim + c]/AMax;
	}
}
...
{
DTED            myDtedFile;
int             r, c, i, j, x1, y1, x2, y2;
int             hillsize;
int             width, height;
int             Zskip;
int             Ix, Iy, sep, numverts;
float           swLat, swLon, neLat, neLon;
float           eyeX  = 250.0, eyeY  = -50.0, eyeZ  = 500.0, eyeD = 10.0;
float           lookX = 200.0, lookY = 200.0, lookZ = 30.0;
float           x[400], y[400]; // z[800][1500];
char            chfile[64];
...
int             Ia, Ib, ii;
short           *Zin;
float           Xview, Yview, Zview, Xlook, Ylook, Zlook;
float           XLEFT, XRITE, YBOT, YTOP, XMIN, XMAX, YMIN, YMAX;
int             LT;
float           dist;
float           xrot, yrot, zrot;

    strcpy(chfile, "n31.dte");
    myDtedFile = dted_open(chfile);
    ...
    Zscale = 2.3;
    Zskip  = 1;
    Ix     = width;    //(width/Zskip)-(Zskip);
    Iy     = height;   //(height/Zskip)-(Zskip);
    if (Ix != Iy) {
      fprintf(stderr, "Ill-formed grid size, x (%f) != y (%f)\n", Ix, Iy);
      exit(1);
    }
//
...
//
    ...
    printf("Skip Factor is .................. %d\n", Zskip);
    printf("Minimum elevation is ............ %f Meters\n", Zmin);
    printf("Maximum elevation is ............ %f Meters\n", Zmax);
    printf("Elevation Scaling is ............ %f\n", Zscale);
    printf("Elevation Bias is ............... %f\n", Zbias);
...
//
    ...
    */
    for (i=1; i<Ix; i++)
      for (j=1; j<Iy; j++) {
        ...
        //y[j] = j+1;
        A[i*GridDim + j] =
	    (float)dted_get_elev(myDtedFile, Zskip*j, Zskip*i);
      }
    ...
    **   Clamp the elevation of the terrain to 0.0 - 1.0
    */
    Zmax = Zmax + Zbias;                                    // Adjust the max value
    for (r = 1; r < GridDim; r++)
      for (c = 1; c < GridDim; c++) {
	A[r*GridDim + c] = A[r*GridDim + c] + Zbias;        // Adjust value by bias
	A[r*GridDim + c] = A[r*GridDim + c]/Zmax;           // Scale value 0.0 - 1.0
	if (A[r*GridDim + c] > 1.0) A[r*GridDim + c] = 1.0; // Clamp height to 1.0
	if (A[r*GridDim + c] < 0.0) A[r*GridDim + c] = 0.0; // Clamp depth to 0.0
	A[r*GridDim + c] = A[r*GridDim + c]*Zscale;         // Put in some exaggeration
...
//
    GridDim = MIN(Ix, Iy);
    fprintf(stderr, "GirDim is %d\n", GridDim);
    A = (float*)calloc(GridDim * GridDim, sizeof(float));
    /*
    **   Initialize elevation to zero
    **/
    for (r = 0; r < GridDim; r++)
       for (c = 0; c < GridDim; c++) A[r*GridDim + c] = 1703.0;
    /*
    **   Build the terrain
    */
    Zmin   = 1000000.0;
...
//
	...
	A[r*GridDim + c] = A[r*GridDim + c] + Zbias;        // Adjust value by bias
	A[r*GridDim + c] = A[r*GridDim + c]/(Zmax);         // Scale value 0.0 - 1.0
	A[r*GridDim + c] = MIN(A[r*GridDim + c], .95);      // Clamp height to 1.0
	A[r*GridDim + c] = MAX(A[r*GridDim + c], 0.0);      // Clamp depth to 0.0
	A[r*GridDim + c] = A[r*GridDim + c]*Zscale;         // Put in some exaggeration
      }
}
