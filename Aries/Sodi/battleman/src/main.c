/* main.f -- translated by f2c (version 19960717).
...
/* Main program */ MAIN__()
{
    ...
    /* Local variables */
    extern /* Subroutine */ int feas_();
    static real tend;
    extern /* Subroutine */ int read_threats__();
    static integer i__, k, itime;
    extern /* Subroutine */ int conin1_();
    static real tbegin;
    extern /* Subroutine */ int matrix_(), fly_threat__(), readdat_(), 
	    getdata_(), wta_(), initial_(), expected_damage__(), 
	    engagement_plan__(), pre_process__();
...
/*        tbegin = etime (tmp) */
	...
	for (itime = 100; i__1 < 0 ? itime >= 2000 : itime <= 2000; itime += 
		i__1) {
	    for (i__ = 1; i__ <= 50; ++i__) {
		if (shared_1.threat_play__[i__ - 1]) {
		    shared_1.war_time__[i__ - 1] += 
			    shared_1.war_time_increment__;
		}
/* L10: */
	    }
...
/*        tend = etime (tmp) */
	s_wsle(&io___4);
	do_lio(&c__9, &c__1, " elapsed time = ", 16L);
	r__1 = tend - tbegin;
...
/* Main program alias */ int gpalsmain_ () { MAIN__ (); }
