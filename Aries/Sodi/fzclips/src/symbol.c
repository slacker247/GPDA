...
#include "utility.h"
#include "argacces.h"
#include "symbol.h"      /* added 03-13-96 */
...
#if FUZZY_DEFTEMPLATES    /* added 03-13-96 */
#define AVERAGE_FUZZY_VALUE_SIZE (sizeof(float) * 10)
#endif
...
/***************************************/
   ...
   static char                   *StringWithinString();/*03-13-96*/
   static int                     CommonPrefixLength();/*03-13-96*/
#endif
...
globle VOID *AddFuzzyValue(fv)
   struct fuzzy_value *fv;
   {
    ...
    if (fv == NULL)
      {
       CLIPSSystemError("FUZZY VALUE",901);  /* what number is correct? 901 arbitrary */
       ExitCLIPS(5);
      }
...
/*   the CLIPSTrueSymbol and CLIPSFalseSymbol.         */
#if FUZZY_DEFTEMPLATES               /* added 03-13-96 */
...
globle VOID InitializeAtomTables()
   {
    ...
    CLIPSTrueSymbol = AddSymbol(TRUE_STRING);
    IncrementSymbolCount(CLIPSTrueSymbol);
    CLIPSFalseSymbol = AddSymbol(FALSE_STRING);
...
globle VOID DecrementFuzzyValueCount(theValue)
  FUZZY_VALUE_HN *theValue;
  {
	if (theValue->count < 0)
	  {
		CLIPSSystemError("FUZZY VALUE",903);
		ExitCLIPS(5);
	  }

	if (theValue->count == 0)
	  {
		CLIPSSystemError("FUZZY VALUE",904);
		ExitCLIPS(5);
	  }
...
/*****************************************************/
globle VOID RefreshSpecialSymbols()    /* changed 03-13-96 */
  {
...
globle SYMBOL_HN *GetNextSymbolMatch(searchString,searchLength,prevSymbol,
  ...
  int anywhere;
  int *commonPrefixLength;   /* added 03-13-96 */
  {
   register int i;
   SYMBOL_HN *hashPtr;
   int flag = CLIPS_TRUE;
   ...
   for (i = 0; i < FUZZY_VALUE_HASH_SIZE; i++)
     {
      ...
      while (fuzzyValuePtr != NULL)
        {
         if ((fuzzyValuePtr->neededFuzzyValue == CLIPS_TRUE) || setAll)
