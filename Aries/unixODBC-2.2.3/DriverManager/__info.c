 ...
 **********************************************************************
 *
 * $Id: __info.c,v 1.21 2002/08/23 09:42:37 lurcher Exp $
 *
 * $Log: __info.c,v $
...
static char const rcsid[]= "$RCSfile: __info.c,v $ $Revision: 1.21 $";
...
int unicode_setup( DMHDBC connection )
{
    ...
    char *be_ucode[] = { "UCS-2-INTERNAL", "UCS-2BE", "UCS-2", NULL };
    char *le_ucode[] = { "UCS-2-INTERNAL", "UCS-2LE", "UCS-2", NULL };
...
static struct state_map state_mapping_2_3[] = {
    { "01S03", "01001" },
    { "01S04", "01001" },
    { "22005", "22018" },
    { "37000", "42000" },
...
char * __wstring_with_length( SQLCHAR *ostr, SQLWCHAR *instr, SQLINTEGER len )
{
    ...
    {
        if ( wide_strlen( instr ) < LOG_MESSAGE_LEN )
        {
            strcpy((char*) ostr, "[" );
    ...
    do
    {
        ...
        if ( SQL_SUCCEEDED( ret ))
        {
            ...
            if ( log_info.log_flag )
            {
                ...
                as2 = (SQLCHAR*) unicode_to_ansi_alloc( msg, SQL_NTS, connection );
        ...
        char *message )
{
    ...
    {
        fp = fopen( log_info.log_file_name, "a" );
    }
