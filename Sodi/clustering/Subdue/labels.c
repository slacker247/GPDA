...
DOUBLE LabelMatchFactor( PGRAPH_VARIABLES GV, 
						 ULONG label1Index, 
						 ULONG label2Index )
{
	MATCH matchType;
	...
	if ( label1Index >= GV->numberOfLabels || label2Index >= GV->numberOfLabels )
		return 1.0; 
	//*/
//	ASSERT( label1Index < GV->numberOfLabels && label2Index < GV->numberOfLabels );
...
/*******************************************************************************
FUNCTION NAME: CountUsedLabels
INPUTS:		PGRAPH_VARIABLES GV, 
RETURNS:	none
PURPOSE:	Count the number of edge and vertex labels actually used after 
			compression.
CALLED BY:	main.c: main()
*******************************************************************************/
/*
void CountUsedLabels(PGRAPH_VARIABLES GV)
{
	...
	graph = GV->graph;
	GV->numberOfUsedVertexLabels = 0;
	GV->numberOfUsedEdgeLabels = 0;
	convTable = (ULONG *)Calloc(GV->numberOfLabels, sizeof(ULONG));
