 ...
 ******************************************************************/
    ...
    static BOOLEAN        is_defuzzify_value_valid(void);
    ...
    static BOOLEAN        is_defuzzify_value_valid();
 ...
 ******************************************************************/
   ...
   /* ALL defuzzify functions must set this variable before exiting. 
      They must set it to CLIPS_TRUE if the defuzzify is valid or
      to CLIPS_FALSE if it is returning a default value when the
      defuzzificztion is undefined (e.g. moment-defuzzify being done
      for a fuzzy set that has zero area)
   */
   static BOOLEAN is_last_defuzzify_valid = CLIPS_TRUE;
           ...
           if (ArgTypeCheck("get-fs-x",2,INTEGER_OR_FLOAT,&result) == CLIPS_TRUE)
             {
               ...
               if (xvalue < from || xvalue > to)
                 { /* error - out of range */
                   PrintCLIPS(WERROR,"Function get-fs-value, x value out of range\n");
                 }
               else
                 {
                   int n, i;
                       ...
                       if (xvalue < fv_ptr->x[0]) /* before 1st point */
                       ...
                       else
                         {
                           ...
                           if (i == n)  /* major problem -- didn't find it! */
                             {
                              PrintCLIPS(WERROR,"Function get-fs-value, internal problem, see system manager\n");
                              SetEvaluationError(CLIPS_TRUE);
                              return( 0.0 );
                             }
  ...
  struct fuzzy_value *fv;  
{
    ...
    is_last_defuzzify_valid = CLIPS_TRUE;
    ...
    if ( num <= 1 )  /* single point is constant fuzzy set over universe */
      {
        ...
        */
        if (num < 1 || fsy[0] == 0.0)
           is_last_defuzzify_valid = CLIPS_FALSE;
      }
    else
      {
        currentx = fsx[0];
      ...
      if (deffun == NULL)
        {
         ...
         fun = FindFunction(modfuncname);
          if (fun == NULL)
            {
              PrintCLIPS(WERROR,"Function add-fuzzy-modifier (undefined modifier function)\n");
              return;
            }
          if ((strcmp(fun->restrictions,"11f") != 0 &&
             ...
             {
               PrintCLIPS(WERROR,"Function add-fuzzy-modifier (incorrect function type in arg 2)\n");
               return;               
             }
        }
#if DEFFUNCTION_CONSTRUCT
        ...
        {    
          ...
          if (deffun->minNumberOfParameters != 1 || deffun->maxNumberOfParameters != 1)
             {
               PrintCLIPS(WERROR,"Function add-fuzzy-modifier (modifier function requires exactly 1 arg)");
               return;               
             }
          ...
          if (!res || (result.type != FLOAT))
             {
               PrintCLIPS(WERROR,"Function add-fuzzy-modifier (modifier function must accept and return FLOAT)");
               return;               
             }
        }
#endif
 
       if (AddFuzzyModifier(modname, NULL, fun, deffun) == CLIPS_TRUE)
   ...
   if (ArgCountCheck("set-fuzzy-inference-type",EXACTLY,1) != -1)
     {     
       ...
       if (strcmp(inference_type,"max-min") == 0 ||
          ...
          {
            PrintCLIPS(WERROR,"Function set-fuzzy-inference-type (argument must be max-min or max-prod)\n");
          }
     }
...
globle VOID set_fuzzy_display_precision()
  {
   ...
   if (ArgCountCheck("set-fuzzy-display-precision",EXACTLY,1) != -1)
     {     
       if (ArgTypeCheck("set-fuzzy-display-precision",1,INTEGER,&arg) == CLIPS_FALSE) 
          return;	    
...
globle VOID plot_fuzzy_value()
  {
   ...
   if (dummyid == NULL)
     {
       ...
       PrintCLIPS(WERROR,"Illegal logical name used for plot-fuzzy-value function.\n");
   ...
   if ((unsigned int)strlen(theString) < 1)
     {
       PrintCLIPS(WERROR,"ERROR: Function plot_fuzzy_value (argument 3 must be a character)\n");
       return;
     }
   ...
   if (from >= to)
     {
       PrintCLIPS(WERROR,"ERROR: Function plot_fuzzy_value (low limit for plot >= high limit)\n");
       return;
     }
   ...
   PrintCLIPS(dummyid, "\nFuzzy Value: ");
   PrintCLIPS(dummyid, ValueToString(fv_ptr->whichDeftemplate->header.name));
   PrintCLIPS(dummyid, "\nLinguistic Value: ");
   ...
   for (fvNum = 0; fvNum <= numArgs-5; fvNum++)
     {
      ...
      if (fv_ptr->whichDeftemplate != fv_ptr_first->whichDeftemplate)
        {
          PrintCLIPS(WERROR,"ERROR: Function plot_fuzzy_value (all fuzzy values must have same fuzzy deftemplate)\n");
          break;
        }

      { char tmpStr[10];

        if (fvNum > 0) PrintCLIPS(dummyid, ",  ");
        PrintCLIPS(dummyid, fv_ptr->name);
        sprintf(tmpStr, " (%c)", theSymbol);
        PrintCLIPS(dummyid, tmpStr);
      }
   ...
   for (i=0; i<=NROWS; i++)
      { sprintf(str, "%5.2f", 1.0 - ((float)i /(float)NROWS));
        PrintCLIPS(dummyid, str);
        PrintCLIPS(dummyid, &PlotSpace[i][0]);
        PrintCLIPS(dummyid, "\n");
      }
        ...
        if (ArgTypeCheck("set-alpha-value",1,INTEGER_OR_FLOAT,&theArgument) != 0)
          {
            ...
            if (theAlphaValue < 0.0 || theAlphaValue > 1.0)
              {
                PrintErrorID("Alpha Value ",909,CLIPS_TRUE);
  ...
  */
  ...
  if ((theDeftemplate == NULL) || 
   ...
   fvExpr = ParseAssertFuzzyFact(logName, &theToken, &error, RPAREN, 
                                 CLIPS_FALSE, theDeftemplate, CLIPS_FALSE);
