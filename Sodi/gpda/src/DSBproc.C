...
//
   ...
   //fd_storyBUDs    = create_form_storyBUDs();
   //create_form_dsbuform1();

   int             nstories = 3;
   float           B[3] = { 80.0, 80.0, 60.0 };
   float           U[3] = {  5.0,  5.0, 40.0 };
   float           D[3] = { 15.0, 15.0,  0.0 };
   char            S[3][3] = { "S1", "S2", "S3" };
   j = -25;
   fd_storyBUDs = fl_bgn_form(FL_FLAT_BOX, 780, 340);
   for (i=0; i<nstories; i++) {
     j = j + 35;
     vobj[i] = fl_add_stacked(FL_VERT_STACKED, j, 30, 30, 310, S[i]);
     fl_set_object_lcolor(vobj[i], FL_CYAN);
     fl_set_object_lsize(vobj[i],  FL_NORMAL_SIZE);
     ...
     sprintf(dsbtemp, "%s\nBelief = %d%%\nIgnorance = %d%%\nConflict = %d%%",
          "CM-01 Neutralized", (int)B[i], (int)U[i], (int)D[i]); 
   ...
   //
   fl_set_xyplot_overlay_type(graph, 1, FL_DASHED_XYPLOT);     // Belief curve 
   //fl_set_xyplot_xbounds(graph, 0.0, );
...
//
   ...
   fclose(EVIDfp);
   //
   if (DSBMYSQL) SQLFreeStmt( hStmt, SQL_DROP );
...
//
   ...
   if (DSBMYSQL) {
     ...
     //
     //   Populate the Story menu with the mission's story names or Master.
     //
     ...
     if ((fp = fopen(dsbtemp, "r")) != NULL) {
       ...
       if (nstory > 0) {
         for (istory=0; istory<nstory; istory++) {
           do fgets(dsbtemp, 128, fp); while (dsbtemp[0] == '#');
           sscanf(dsbtemp, "%*s %*f %*f %*f %s", stories[numStories]);

           // Strip the leading path.
           ptr = strrchr(stories[numStories], '/');
           if (ptr != NULL)
             strcpy(dsbtemp, ptr + 1);
           else
             strcpy(dsbtemp, stories[numStories]);
         ...
         fl_addto_choice(fd_network->dsb_story, "Master");
     }
   }
...
void showStoryBrowser()
{
   ...
   //   Get all the tables
   //
   if ( ExecuteSQL(DSBhDbc, "show tables;",
                   cDelimiter, bColumnNames, bHTMLTable) > 0 ) {
     nReturn = SQLFetch( hStmt );
     while ( nReturn == SQL_SUCCESS || nReturn == SQL_SUCCESS_WITH_INFO ) {
   ...
   fl_show_form(fd_storyBrowser->storyBrowser, FL_PLACE_CENTER, FL_FULLBORDER,
                "Story Browser");
...
fl_do_forms();
     ...
     case 2: // Browser list callback
       fl_set_input(fd_storyBrowser->input,
                    fl_get_browser_line(fd_storyBrowser->list,
                                        fl_get_browser(fd_storyBrowser->list)));       break;
   }
}
...
"Edit %t|Assessment|Stories|New Evidence|Node Override|Node Copy|Node Paste|Link Edit|Influence");
     //
...
char            chfile[128];
   ...
   switch (item_no) {
     ...
     case 9: // Get 'Help' selection
       k = 0; /*fl_get_menu(fd_network->nethelp_menu)-1;*/
...
float           disbelief, confidence, Tin;
   ...
   browser_line = fl_get_browser(fd_dsevedit->input_browser) - 1;
   ...
   if (item_no == 1) {                            // Do 'Apply'
     ...
     if (!browser_mod) {
       ...
       fl_replace_browser_line(fd_dsevedit->input_browser,
                               fl_get_browser(fd_dsevedit->input_browser), chtemp);
     }
     EvidModified = TRUE;
...
float           degrade;
   ...
   default:
     ...
     fl_set_choice(fd_dsevedit->degrade, EVID_NONE+1);
     degrade = 1.0;
     break;
   }
...
******/
       ...
       U = 2.0;
/****** ttt -- old code
       numNodes = DS_AlgoGetLevelWidth(level - 1);
       for (i=0; i < numNodes; i++) {
         DS_AlgoGetResult(i, level-1, results);
...
//
     fl_deactivate_object(fd_dsevedit->prop_but);
     fl_set_object_color(fd_dsevedit->prop_but, FL_INACTIVE_COL, FL_INACTIVE_COL);
