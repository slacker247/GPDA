...
int main( int argc, char *argv[] )
{
	...
	printf( "\tCPU time used by SUBDUE = %7.3f sec\n\n", 
		...
		else
		{
			...
			// saving the sub definition
			if (positiveGraph->saveBestSubDef && bestSub->definition != NULL) {
				SaveSub(positiveGraph, bestSub);
			}
					  ...
					  BOOLEAN *supervised, char **predefSubsFileName )
{
	...
	while ( ( --argc > 0 ) && ( (*++argv)[0] == '-' ) )
	{
		...
		else if ( strcmp( *argv, CLA_SAVE_SUB ) == 0 )							// save sub definition
		{
			GV->saveBestSubDef = TRUE;
		}
...
void LoadDefaultGraphParameters(PGRAPH_VARIABLES graph)
{
	...
	graph->prune = FALSE;
	graph->minEncode = TRUE;						// this cannot be set to false anymore
...
void Usage( void )
{
	...
	printf("              [%s] [%s] [%s #] [%s #] [%s]\n", CLA_NSUBS, CLA_PRUNE, CLA_PRUNE2, CLA_BEAM, CLA_OVERLAP);
	printf("              [%s] [%s connectivity] [%s compactness]\n", CLA_UNDIRECT, CLA_CONNECTIVITY, CLA_COMPACTNESS);
	printf("              [%s coverage] [%s] [%s filename ]\n", CLA_COVERAGE, CLA_ALT_GROUPS, CLA_PREDEF_SUBS);
	printf("              [%s min max] [%s level] [%s processCount]\n", CLA_SIZE, CLA_OUTPUT, CLA_NPROC);
	printf("              [%s plotFile] [%s] [%s]\n", CLA_PLOT, CLA_EVAL_TYPE, CLA_SAVE_SUB);
	printf("              [%s] [%s] [%s]\n", CLA_CLUSTER, CLA_TRUE_LABEL, CLA_EXHAUST);
	printf("              [%s] [%s] [%s numpos]\n", CLA_SUPERVISED, CLA_SCRATCH, CLA_POSITIVE_EXAMPLES);
	printf("              [%s percent] [%s weight]\n", CLA_POS_EX_RATIO, CLA_NEG_WEIGHT);
	printf("              graphfile\n");
