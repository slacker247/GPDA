/**********************************************************************
 See GR_Tearth2.C
  ...
  -- 07/22/93: changed "exit" to be "return" on file not found.
...
void three_comp_rgbtocpack 
     (unsigned short* r, unsigned short* g, unsigned short* b,
      unsigned long* l, int n);
*/
GR_Tearth2::GR_Tearth2 (char *dirname, long type)
{
   int i, j, maxi;
   ...
   for (i=0; i<14; i++)
   {
      sprintf (p_filename[i], "%s/tearth2data/te.%d", dirname, i+1);
      printf ("%d). --> %s\n", i+1, p_filename[i]); 
   }
   ...
   for (i=0; i<2 && i < maxi; i++)
   {
     texbind (TX_TEXTURE_0, 11+i);
     draw_area (-180, -90+150*i, 180, -60+150*i); 
   }
   /* 
   texbind (TX_TEXTURE_0, 11);
   draw_area (-180, -90, 180, -60);
   texbind (TX_TEXTURE_0, 12);
   draw_area (-180, 60, 180, 90);
   */

   for (j=0; j<=1 && (j*6+i)<maxi; j++)
   {
      for (i=0; i<=5 && (j*6+i)<maxi; i++)
      {
	 texbind (TX_TEXTURE_0, j*6+i+13);
	 draw_area (-180+i*60, -60+j*60, -120+i*60, j*60);
      }
   }
   GR_closeobj ();
}
...
int
GR_Tearth2::texture_init ()
{
   unsigned long *image[14];
   int i, maxi;
   char msgstr[80];
   ...
   for (i=0; i<14; i++)
   {
      ...
      if (!image[i])
      {
         fprintf (stderr, "Oops, I don't find image file %s.\n",
                  p_filename[i]);
         work_progress (3, NULL); 
         return i;
      } 
      texdef2d(11+i,4,p_xsize,p_ysize,image[i],7,p_texps);
      maxi = i; 
   }
   maxi++;
...
void
GR_Tearth2::draw_area (long slon, long slat, long elon, long elat)
{
   int j, k;
   long jmax, kmax;
   float theta, phi;
...
unsigned long*
GR_Tearth2::take_rgbdata (char* infile)
{
   IMAGE* image;
   int xsize, ysize;
