...
int             NumCells = 100;         /* 36 Terrain grid is NumCells X NumCells cells */
...
int             Init_pos = GL_TRUE;     /* If true, set initial position and kbd mode */
float           Init_x = 1.0,           /* Initial X is at West edge of terrain */
                Init_y = 1.0,           /* Initial Y is at South edge of terrain */
                Init_z = 1.5,           /* Initial altitude is 1.5 unit above terrain */
                Init_azimuth = 0.0;     /* Initial bearing is due North */
int             rgbmode = GL_TRUE;
...
void init_skyfly(int terrain_type)
{
    init_shmem();
    init_gfxpipes();
    init_clouds();
    init_terrain(terrain_type);
    init_paper_planes();
    init_positions();
...
void cull_proc(void)
{
    ...
    if (!init) {
        ...
        ringbuffer.ring = (perfobj_t **)malloc(RING_SIZE * sizeof(perfobj_t *));
...
float       cntry = (y +.5) * CellSize;
	    ...
	    if ((left_area < cull.epsilon * cull.side && right_area >
		 -cull.epsilon * cull.side)) {
	...
	*/
	for (i = 0; i < NUM_PLANES; i++) {
	  ...
	  if (left_area < cull.plane_epsilon * cull.side && right_area >
...
void init_gl(int gfxpipenum)
{
   ...
   gluPerspective(FOV*RAD_TO_DEG, (float)Wxsize/(float)Wysize, 0.1, far_cull *0.95);
   ...
   if (rgbmode) {
     ...
     if (fog && fog_density > 0) glEnable(GL_FOG);
...
int             w, h;
   ...
   if (!threecomp) {
     ...
     */
     ...
     if (!mipmap) glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, texmode);
     glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, texmode);
     if (mipmap)
       gluBuild2DMipmaps(GL_TEXTURE_2D, /*0,*/ 1, 256, 256, /*0,*/
			 GL_LUMINANCE, GL_UNSIGNED_BYTE, bwimage256);
     else if (rgbmode)
       glTexImage2D(GL_TEXTURE_2D, 0, 1, 256, 256, 0,
		    GL_LUMINANCE, GL_UNSIGNED_BYTE, bwimage256);
       else {
#define TXSIZE 128
...
int ii;
	 ...
	 for (ii = 0; ii < TXSIZE*TXSIZE; ii++) {
	   buf[ii] = terr_base + FOG_LEVELS * (buf[ii] >> (8-TERR_BITS));
	 }
...
#undef TXSIZE
	  ...
	  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, texmode);
       glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, texmode);
       if (mipmap)
	 gluBuild2DMipmaps(GL_TEXTURE_2D, /*0,*/ 1, 128, 128, /*0,*/
			   GL_LUMINANCE, GL_UNSIGNED_BYTE, bwimage128);
       else if (rgbmode)
	 glTexImage2D(GL_TEXTURE_2D, 0, 1, 128, 128, 0,
		      GL_LUMINANCE, GL_UNSIGNED_BYTE, bwimage128);
	 else {
#define TXSIZE 64
GLubyte buf[TXSIZE*TXSIZE];
int ii;
	   gluScaleImage(GL_LUMINANCE, 128, 128, GL_UNSIGNED_BYTE, bwimage128,
			 TXSIZE, TXSIZE, GL_UNSIGNED_BYTE, buf);
	   for (ii = 0; ii < TXSIZE*TXSIZE; ii++) {
	      buf[ii] = sky_base + FOG_LEVELS * (buf[ii] >> (8-SKY_BITS));
	   }
#ifdef GL_COLOR_INDEX8_EXT  /* Requires SGI_index_texture and EXT_paletted_texture */
	   glTexImage2D(GL_TEXTURE_2D, 0, GL_COLOR_INDEX8_EXT, TXSIZE, TXSIZE,
			0, GL_COLOR_INDEX, GL_UNSIGNED_BYTE, buf);
