 ...
 **********************************************************************
 *
 * $Id: SQLConnect.c,v 1.15 2002/08/23 09:42:37 lurcher Exp $
 *
 * $Log: SQLConnect.c,v $
...
static char const rcsid[]= "$RCSfile: SQLConnect.c,v $ $Revision: 1.15 $";
...
int __connect_part_one( DMHDBC connection, char *driver_lib, char *driver_name, int *warnings )
{
    ...
    *warnings = FALSE;
    ...
    SQLGetPrivateProfileString( driver_name, "DontDLClose", "0",
				mapping_string, sizeof( mapping_string ), 
                "ODBCINST.INI" );
    ...
    if ( SQL_SUCCEEDED( ret ))
    {
        ...
        if ( ptr -> connection.driver_version == SQL_OV_ODBC3 )
        {
            if ( CHECK_SQLFREEHANDLE(( &ptr -> connection )))
            {
                SQLFREEHANDLE(( &ptr -> connection ),
                        SQL_HANDLE_ENV,
                        ptr -> connection.driver_env );
            }
            else if ( CHECK_SQLFREEENV(( &ptr -> connection )))
            {
                SQLFREEENV(( &ptr -> connection ),
                        ptr -> connection.driver_env );
            }
        }
        else
        {
            if ( CHECK_SQLFREEENV(( &ptr -> connection )))
            {
                SQLFREEENV(( &ptr -> connection ),
                        ptr -> connection.driver_env );
            }
